...
class FabrikHtml
{
	 ...
	 * @var array
	 */
	public static $jLayoutsJs = array();
...
EOD;
	...
	public static function emailForm($formModel, $template = '')
	{
		...
		$form                  = $formModel->getForm();// F5: Call to a member function getForm() on null
	...
	public static function printURL($formModel)
	{
		...
		else
		{
			...
			// F5: view=details: there is no details view, must add task=details.display
			$url = COM_FABRIK_LIVESITE . 'index.php?option=com_fabrik&view=details&tmpl=component&formid=' . $form->id . '&listid=' . $table->id
				. '&rowid=' . $formModel->getRowId() . '&iframe=1&print=1';
	...
	public static function conditionList($listId, $sel = '')
	{
		...
		return HTMLHelper::_('select.genericlist', $conditions, $name, 'class="form-select-sm" size="1" ', 'value', 'text', $sel);
	}
	...
	/**
	 * Fabrik to load in a style sheet
	 ...
	 * @param   string $file    stylesheet asset name to set in asset register
	 * @param   array  $attribs not used
	 *
	 * @return  null
	 */
	public static function stylesheet($file, $assetName, $attribs = array())
	{
		//$wa = $this->document->getWebAssetManager();// not in scope here
		...
		if (self::cssAsAsset()) // F5: checks raw format (should have a more descriptive name like checkraw() ) do we ever have raw format?
		{
			if (!in_array($file, self::$ajaxCssFiles))
			{
				if (!strstr($file, 'fabrik.css'))
				{
					$wa->registerAndUseStyle($assetName, $file);
...
//	public static function stylesheetFromPath($path)
	public static function stylesheetFromPath($path, $assetName)
	{
		if (strstr($path, '?'))
		{
			$file = explode('?', $path);
...
//			self::stylesheet($path);
			self::stylesheet($path, $assetName);
	...
	public static function mcl()
	{
		...
		$src = array(
			'/lib/mcl/CANVAS.js',
			'/lib/mcl/CanvasItem.js',
			'/lib/mcl/Cmorph.js',
			'/lib/mcl/Layer.js',
			'/lib/mcl/LayerHash.js',
			'/lib/mcl/Thread.js'
		);
	...
	public static function framework()
	{
		...
		{
			$app     = Factory::getApplication();
			FabrikHtml::modalLayoutInterfaces();
			$liveSiteSrc = array();
			$liveSiteReq = array();
...
/*
			...
			if (!self::inAjaxLoadedPage())
			{
				...
				self::styleSheet(COM_FABRIK_LIVESITE . 'media/com_fabrik/css/fabrik.css','fabrik');
...
*/
	...
	public static function slimbox()
	{
		...
		if (!self::$modal)
		{
			...
			if ($fbConfig->get('use_mediabox', 1))
			{
				...
				self::script('/lib/mediabox-advanced/mediaboxAdv.js');
			}
			else
			{
					HTMLHelper::stylesheet('components/com_fabrik/libs/slimbox2/css/slimbox2.css');
					self::script('/lib/slimbox2/slimbox2.js');
			}
	...
	public static function slideshow()
	{
		...
		self::script('/lib/slick/slick.js');
		FabrikHtml::stylesheet(COM_FABRIK_LIVESITE . 'media/com_fabrik/js/lib/slick/slick.css');
		FabrikHtml::stylesheet(COM_FABRIK_LIVESITE . 'media/com_fabrik/js/lib/slick/slick-theme.css');

		$folder = 'media/com_fabrik/js/lib/elevatezoom-plus/';
		$ext = self::isDebug() ? '.js' : '.js';
		self::script('/lib/elevatezoom-plus/jquery.ez-plus.js');
	}
	...
	public static function atWho($selector, $placeHolders = array())
	{
		...
		if (!array_key_exists($key, self::$atWho))
		{
			...
			FabrikHtml::stylesheet('media/com_fabrik/js/lib/at/' . $css);
...
EOT;
	...
	public static function image($file, $type = 'form', $tmpl = '', $properties = array(), $srcOnly = false, $opts = array())
	{
		...
		if ($forceImage !== true)
		{
			...
			if (!$srcOnly)
			{
				return FabrikHtml::icon($class, '', $p);
			}
			else
			{
				return $class;
	...
	{
		$icon = FabrikHtml::getLayout('fabrik-icon')
            ->render((object) array(
                    'icon' => $icon,
	...
	{
		FabrikHtml::jLayoutJs('modal-close', 'modal.fabrik-close');
		FabrikHtml::jLayoutJs('icon-expand', 'fabrik-icon', (object) array('icon' => 'icon-expand'));
		FabrikHtml::jLayoutJs('icon-full-screen', 'fabrik-icon', (object) array('icon' => 'icon-out-2 icon-fullscreen'));
	}
