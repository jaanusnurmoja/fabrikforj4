...
use Fabrik\Library\Fabrik\FabrikCipher;
...
class FabrikWorker {
	 ...
	 * @since  3.1
	 *
	 * @return  Fabrik\Helpers\FabrikCipher
	 */
	public static function getCrypt($type = 'simple') {
		return new FabrikCipher($type);
	}
...
//print_r("1 ");print_r($returnType);print_r(" 2 ");print_r($messages);print_r(" 3 ");print_r(ArrayHelper::getValue($messages, 0, ''));exit;
	 ...
	 */
//	Change to only use this for foreign database, not for joomla database $loadJoomlaDb is always false, $cnnId is required
	public static function getDbo($loadJoomlaDb = false, $cnnId = null) {
		...
		if (!array_key_exists($sig, self::$database)) {
			Table::addIncludePath(JPATH_ADMINISTRATOR . '/components/com_fabrik/src/Table');
			$conf = Factory::getApplication()->getConfig();

			if (!$loadJoomlaDb) {
				$cnModel = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Connection', 'Site');
				$cn = $cnModel->getConnection($cnnId); // Same function is here also, why take it from site connection?
				$host = $cn->host;
				$user = $cn->user;
			...
			// All getInstance are deprecated, do we now need to use:
			// self::$database[$sig] = Factory::getContainer()->get(DatabaseInterface::class);
	...
	public static function getConnection($item = null) {
		...
		if (!array_key_exists($connId, self::$connection)) {
			$connectionModel = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Connection', 'Site');
	 ...
	 */
//	Does not get called when needed, so keep it or remove ?
	public static function getPluginManager() {
		if (!self::$pluginManager) {
			self::$pluginManager = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Pluginmanager', 'Site');
		}
		...
		if (!$app->isClient('administrator')) {
			...
			if (!is_null($listId)) {
				if (!array_key_exists($listId, $listIds)) {
					$db = Factory::getDbo();
					// Deprecated, do we now need to use: $db = Factory::getContainer()->get(DatabaseInterface::class);
					// $db = $this->getDatabase(); can not be used here, because class does not extends from a joomla class
	...
	public static function canUserDo($params, $row, $col) {
		if (!is_null($row)) {
			...
			if($userCol === '0') $userCol = ''; // F5: "None" = '0', so change it here
	...
	public static function formDefaults($form) {
		...
		Form::addFieldPath(JPATH_COMPONENT . '/src/Field');
	...
	/**
	 * Get the webAssetManager instance and set our media path
	 *
	 * @return  object
	 */
	public static function getWebAssetManager() {
		...
		$wa->getRegistry()->addExtensionRegistryFile('fabrikar/com_fabrik');
