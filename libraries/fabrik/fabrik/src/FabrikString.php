 ...
 * @since       3.0
 */
class FabrikString extends StringHelper {
	/**
	...
	public static function ltrimword($str, $word = false, $whitespace = false) {
		...
		if ($whitespace) {
			...
			$pos = JoomlaStringHelper::strpos($str, $word);

			if ($pos === 0) {
				$str = JoomlaStringHelper::substr($str, JoomlaStringHelper::strlen($word));
			}
		}
	...
	public static function rtrimword(&$str, $word = false) {
		$l = JoomlaStringHelper::strlen($word);
		$end = JoomlaStringHelper::substr($str, -$l);

		if ($end === $word) {
			return JoomlaStringHelper::substr($str, 0, JoomlaStringHelper::strlen($str) - $l);
		} else {
			return $str;
		}
	}
	...
	public static function getShortDdLabel($label) {
		...
		if (JoomlaStringHelper::strlen($label) > 50) {
			$label = JoomlaStringHelper::substr($label, 0, 47) . '...';
		}
	...
	public static function iclean($str, $fromEnc = "UTF-8", $toEnc = "ASCII//IGNORE//TRANSLIT") {
		...
		for ($i = 0; $i < JoomlaStringHelper::strlen($str); $i++) {
			$ch = ord($str[$i]);
	...
	public static function clean($str, $fromEnc = "UTF-8", $toEnc = "ASCII//IGNORE//TRANSLIT") {
		return JoomlaStringHelper::strtolower(self::iclean($str, $fromEnc, $toEnc));
	}
...
//	Same function is also in Joomla\CMS\HTML\Helpers\StringHelper
	public static function fabtruncate($text, $opts = array()) {
		$origText = $text;
	...
	public static function bitlify($link, $login, $apikey, $encode = true) {
		if (!strstr($link, 'bit.ly/') && $link !== '') {
			...
			if ($api->isCurlError()) {
				FabrikWorker::log('fabrik.helpers.bitlify.error', $api->getCurlErrno() . ': ' . $api->getCurlError());
			} else {
				if ($result->isError()) {
					FabrikWorker::log('fabrik.helpers.bitlify.error', print($result->getResponse()));
				} else {
					if ($result->isSuccess()) {
