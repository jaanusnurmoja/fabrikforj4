...
<div class="row justify-content-between pb-3 pe-3">
	...
	<div class=" col-auto fabrikButtonsContainer">
		<div class="row row-cols-auto align-items-start">
			<?php if ($this->showAdd) :?>
				<div class="col px-0">
					<a class="addbutton addRecord btn " href="<?php echo $this->addRecordLink;?>">
						<?php echo FabrikHtml::icon('icon-plus', $this->addLabel);?>
					</a>
				</div>
			<?php endif; ?>
			...
			<?php endif; ?>
			<?php if ($this->showGroupBy) : ?>
				<?php if ($this->canGroupBy) : ?>
					<div class="col px-0">
						<?php
						$displayData = new \stdClass;
						$displayData->icon = FabrikHtml::icon('icon-list-view');
			...
			if (($this->showClearFilters && (($this->filterMode === 3 || $this->filterMode === 4))  || $this->bootShowFilters == false)) :
				...
				<div class="col px-0">
					<a class="btn  <?php echo $clearFiltersClass; ?>" href="#">
						<?php echo FabrikHtml::icon('icon-undo', Text::_('COM_FABRIK_CLEAR'));?>
					</a>
				</div>
			<?php endif;
			...
			if ($this->advancedSearch !== '') : ?>
				<div class="col px-0">
					<a href="<?php echo $this->advancedSearchURL?>" class="advanced-search-link btn ">
						<?php echo FabrikHtml::icon('icon-search', Text::_('COM_FABRIK_ADVANCED_SEARCH'));?>
					</a>
				</div>
			<?php endif;
			...
			<div class="col px-0" >
				<?php
				$displayData = new \stdClass;
				$displayData->icon = FabrikHtml::icon('icon-upload');
				$displayData->label = Text::_('COM_FABRIK_CSV');
				$displayData->links = array();
				if ($this->showCSVImport) :
					$displayData->links[] = '<a href="' . $this->csvImportLink . '" class="csvImportButton nav-link">' . FabrikHtml::icon('icon-download', Text::_('COM_FABRIK_IMPORT_FROM_CSV'))  . '</a>';
				endif;
				if ($this->showCSV) :
					$displayData->links[] = '<a href="#" class="csvExportButton nav-link">' . FabrikHtml::icon('icon-upload', Text::_('COM_FABRIK_EXPORT_TO_CSV')) . '</a>';
				endif;
				$layout = $this->getModel()->getLayout('fabrik-nav-dropdown');
				echo $layout->render($displayData);
				?>
			...
			<div class="col px-0">
					<a href="<?php echo $this->rssLink;?>" class="feedButton">
						<div class="row row-cols-auto">
							<div class="col pe-0"><?php echo FabrikHtml::image('feed.png', 'list', $this->tmpl);?></div>
			...
			if ($this->emptyLink) :?>
				<div class="col px-0">
					<a class="doempty btn " href="<?php echo $this->emptyLink;?>">
						<?php echo FabrikHtml::icon('icon-ban', Text::_('COM_FABRIK_EMPTY'));?>
					</a>
				</div>
			<?php
			endif;
