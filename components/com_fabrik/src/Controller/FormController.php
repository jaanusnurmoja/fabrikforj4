...
{
$model = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('FormInlineEdit', 'Site');
$model->render();
}
 */
	/**
	 ...
	 *
	 * @return  Controller|void  A Controller object to support chaining.
	 * F5: info, this is called if task=form.view
	 */
	public function display($cachable = false, $urlparams = array()) {
		...
		$view = $this->getView($viewName, $viewType, 'Site');

		// Push a model into the view (may have been set in content plugin already)
		// @var ModelForm $model
		$model = !isset($this->_model) ? $this->getModel($modelName, 'Site') : $this->_model;
		...
		if (!$this->isMambot && $model->checkAccessFromListSettings() <= 1) {
			...
			if ($app->isClient('administrator')) {
				...
				// F5: view=details: there is no details view
		...
		if (!FabrikWorker::useCache($listModel)
			...
			FabrikHtml::addToSessionCacheIds($cacheId);
	...
	public function process() {
		...
		$viewName = $input->get('view', 'form');
		$view = $this->getView($viewName, Factory::getDocument()->getType(), 'Site');

		/** @var ModelForm $model */
		if ($model = $this->getModel('form', 'Site')) {
			$view->setModel($model, true);
		}
		...
		if ($input->getInt('elid', 0) !== 0) {
			...
			$inlineModel = $this->getModel('forminlineedit', 'Site');
	...
	public function savepage() {
		$app = Factory::getApplication();
		$input = $app->getInput();
		$model = $this->getModel('Formsession', 'Site');
		$formModel = $this->getModel('Form', 'Site');
	...
	public function clearSession() {
		...
		$sessionModel = $this->getModel('formsession', 'Site');
