 ...
 * @since       3.3.4
 */
class AdvancedSearchModel extends FabSiteModel 
{
    /**
    ...
    public function opts()
    {
        ...
        $opts->conditionList = FabrikHtml::conditionList($listRef, '');
        list($fieldNames, $firstFilter) = $this->getAdvancedSearchElementList();
        $statements = $this->getStatementsOpts();
        $opts->elementList = HTMLHelper::_(
            'select.genericlist',
            $fieldNames,
            'fabrik___filter[list_' . $listRef . '][key][]',
            'class="form-select-sm key" size="1" ',
            'value',
            'text'
        );
        $opts->statementList = HTMLHelper::_(
            'select.genericlist',
            $statements,
            'fabrik___filter[list_' . $listRef . '][condition][]',
    ...
    private function getAdvancedSearchElementList()
    {
        ...
        foreach ($elementModels as $elementModel) {
			...
			if ($plugin !== 'date' || $plugin !== 'jdate') { // (Temporary) Exclude date & jdate
    ...
    public function getAdvancedSearchRows()
    {
        ...
        // F5: By join they mean the OPERATOR (and / or)
        if (array_key_exists('key', $filters)) {
            foreach ($filters['key'] as $key) {
                ...
                if ($counter == 0) {
                    ...
                    $join = FabrikHtml::conditionList($model->getRenderContext(), $join);
                }
    ...
    public function filterValues()
    {
        ...
        $advanced = array();
        $iKeys = array_keys(FabrikArray::getValue($filters, 'key', array()));

        foreach ($iKeys as $i) {
            $searchType = FabrikArray::getValue($filters['search_type'], $i);
            ...
            if (!is_null($searchType) && $searchType == 'advanced') {
                foreach (array_keys($filters) as $k) {
                    if (array_key_exists($k, $advanced)) {
                        ...
                        $kf = FabrikArray::getValue($filters, $k, array());
                        if (is_array($advanced[$k])) {
                            $advanced[$k][] = FabrikArray::getValue($kf, $i, '');
                        }
                    } else {
    ...
    public function link()
    {
        ...
        if ($params->get('advanced-filter', '0')) {
            ...
            $layout = FabrikHtml::getLayout('list.fabrik-advanced-search-button');// is also in default_buttons
    ...
    public function url()
    {
        ...
        $url = COM_FABRIK_LIVESITE . 'index.php?option=com_fabrik&amp;format=partial&amp;view=list&amp;layout=_advancedsearch&amp;tmpl=component&amp;listid='
