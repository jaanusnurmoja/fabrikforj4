 ...
 * @since    3.0
 */
class PluginelementModel extends PluginModel {
	/**
	 * Element id
	 *
	 * @var int
	...
	public function __construct(&$subject, $config = array()) {
		...
		$this->validator = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Elementvalidator', 'Site');
...
//			Table::addIncludePath(JPATH_ADMINISTRATOR . '/components/com_fabrik/tables');
			$row = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createTable('Element', 'Administrator');
	...
	public function getParent() {
		if (!isset($this->parent)) {
			...
			if ((int) $element->parent_id !== 0) {
				$this->parent = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createTable('Element', 'Administrator');
//				$this->parent = FabTable::getInstance('element', 'FabrikTable');
	...
	public function bindToElement(&$row) {
		if (!$this->element) {
//			Table::addIncludePath(JPATH_ADMINISTRATOR . '/components/com_fabrik/tables');
			$this->element = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createTable('Element', 'Administrator');
//			$this->element = FabTable::getInstance('Element', 'FabrikTable');
	 ...
	 *
	 * @param   ModelGroup &$groupModel group model
	 * @param   ModelForm  &$formModel  form model
	 * @param   ModelList  &$listModel  list model
	 *
	 * @return  void
	 */
	...
	public function &getGroup($groupId = null) {
		...
		if (is_null($this->group) || $this->group->getId() != $groupId) {
			$model = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Group', 'Site');
			$model->setId($groupId);
			$model->getGroup();
			$this->group = $model;
		}
	 ...
	 * Set the group model
	 *
	 * @param   ModelGroup $group group model
	 *
	 * @since 3.0.6
	 *
	 * @return  null
	...
	public function getFormModel() {
		if (is_null($this->form)) {
			...
			$this->form = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Form', 'Site');
	 ...
	 * Set form model
	 *
	 * @param   ModelForm $model form model
	 *
	 * @return  void
	 */
	...
	protected function replaceWithIcons($data, $view = 'list', $tmpl = null) {
		...
		if ($iconFile === '{extension}') {
			...
			$iconFile = FabrikArray::getValue($iconFileInfo, 'extension', '');
		}
		...
		foreach ($cleanDatas as $cleanData) {
			foreach ($this->imageExtensions as $ex) {
				$img = FabrikHtml::image($cleanData . '.' . $ex, $view, $tmpl, array(), false, $opts);
			...
			if (empty($img) && $iconFolder === 2) {
				...
				$img = FabrikHtml::image($cleanData, $view, $tmpl, array(), false, $opts);
			}
			...
			if (!empty($img)) {
				...
				if ($params->get('icon_hovertext', true)) {
					...
					$layout = FabrikHtml::getLayout('element.fabrik-element-listicon-tip');
					...
					if ($as->length) {
						...
						$src = FabrikHtml::image($cleanData . '.' . $ex, $view, $tmpl, array(), true, array('forceImage' => true));
	...
	public function getAsField_html(&$aFields, &$aAsFields, $opts = array()) {
		...
		$fullElName = FabrikArray::getValue($opts, 'alias', $db->qn($dbTable . '___' . $this->element->name));
		...
		if ($this->isJoin()) {
			...
			$joinTable = $this->getJoinModel()->getJoin()->table_join;
			$fullElName = FabrikArray::getValue($opts, 'alias', $fullElName);
	...
	*/
		...
		if (!isset($this->default)) {
			...
			if ($element->eval == "1" && is_string($default)) {
				...
				if (!stristr($default, 'return')) {
					...
					FabrikHtml::debug($default, 'element eval default:' . $element->label);
	...
	public function setValuesFromEncryt(&$post, $key, $data) {
		FabrikArray::setValue($post, $key, $data);
		FabrikArray::setValue($_REQUEST, $key, $data);
	...
	protected function getDefaultOnACL($data, $opts) {
		...
		if (!$this->isEditable() && (int) FabrikArray::getValue($data, 'rowid') != 0) {
			// F4 update
 ...
 */
	public function getValue($data, $repeatCounter = 0, $opts = array()) {
		...
		if (!array_key_exists($key, $this->defaults)) {
			...
			if (FabrikArray::getValue($opts, 'raw', 0) == 1) {
				$name .= '_raw';
			}
			...
			if ($groupRepeat) {
				...
				$values = FabrikArray::getValue($values, $repeatCounter, '');
			}

			if (FabrikArray::getValue($opts, 'runplugins', false)) {
				$formModel = $this->getFormModel();
 ...
 */
	public function getLabel($repeatCounter, $tmpl = '') {
		...
		if ($displayData->rollOver) {
			$displayData->icons .= FabrikHtml::image('question-sign', 'form', $tmpl, $iconOpts) . ' ';
		}
		...
		$layout = FabrikHtml::getLayout('fabrik-element-label', $this->labelPaths());
 ...
 */
	protected function addErrorHTML($repeatCounter, $tmpl = '') {
		...
		$layout = FabrikHtml::getLayout('element.fabrik-element-error');
		$displayData = new \stdClass;
		$displayData->err = $err;
 ...
 */
	protected function rollover($txt, $data = array(), $mode = 'form') {
		...
		if (!empty($title)) {
			$layout = FabrikHtml::getLayout('element.fabrik-element-tip');
			$displayData = new \stdClass;
 ...
 */
	protected function tipTextAndValidations($mode, $data = array()) {
		...
		if ($this->isTipped($mode, $data)) {
			$lines[] = '<li>' . FabrikHtml::image('question-sign', 'form', $tmpl) . ' ' . $this->getTipText($data) . '</li>';
		}
 ...
 */
	public function copyRow($id, $copyText = 'Copy of %s', $groupId = null, $name = null) {
		...
		$rule = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createTable('Element', 'Administrator');
//		$rule = FabTable::getInstance('Element', 'FabrikTable');
		...
		if (is_a($this, 'PlgFabrik_ElementDatabasejoin')) {
			$join = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createTable('Join', 'Administrator');
//			$join = FabTable::getInstance('Join', 'FabrikTable');
		...
		foreach ($actions as $id) {
			$jsCode = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createTable('Jsaction', 'Administrator');
 ...
 */
	public function preRender($c, $elCount, $tmpl) {
		...
		if ($tip !== '') {
			$tip = FabrikHtml::image('question-sign', 'form', $tmpl) . ' ' . $tip;
		}
 ...
 */
	protected function containerClass($element) {
		...
		$pluginResults = FabrikWorker::getPluginManager()->runPlugins(
 ...
 */
	protected function inputProperties($repeatCounter, $type = null) {
		...
		if (!isset($type)) {
			...
			switch ($params->get('password')) {
			// Kept the name 'password' for backward compatibility
			case '1':
				$type = 'password';
				break;
			case '2':
...
// Cant be used with hidden element types
		...
		if (!array_key_exists((int) $repeatCounter, $this->HTMLids)) {
			...
			if ($groupModel->isJoin()) {
				...
				if (empty($table->db_table_name)) {
					$table->db_table_name = 'form' . $groupModel->form->id;
				}
	...
	protected function getDefaultFilterVal($normal = true, $counter = 0) {
		...
		if ($default == '') {
			...
			if (array_key_exists('elementid', $filters)) {
				...
				if ($k !== false) {
					$searchType = FabrikArray::getValue($filters['search_type'], $k);
					...
					if ($searchType != 'advanced' || $filters['key'][$k] === $input->getString('element')) {
						...
						if (!is_null($searchType) && $searchType != 'searchall') {
							if ($searchType != 'prefilter') {
								$default = FabrikArray::getValue($filters['origvalue'], $k);
							}
						}
					}
				}
	...
	protected function autoCompleteFilter($default, $v, $labelValue = null, $normal = true, $container = null) {
		...
		FabrikHtml::autoComplete($selector, $element->id, $formId, $element->plugin, $opts);
		...
		for ($j = $c; $j >= 0; $j--) {
			...
			if (is_array($vals)) {
				for ($i = 0; $i < count($vals); $i++) {
					...
					$txt2 = FabrikWorker::JSONtoData(FabrikArray::getValue($txt, $i), true);
	...
	protected function filterValueList_Exact($normal, $tableName = '', $label = '', $id = '', $incJoin = true) {
		...
		FabrikHtml::debug((string) $fabrikDb->getQuery(), 'element filterValueList_Exact:');
	...
	protected function getFilterHiddenFields($counter, $elName, $hidden = false, $normal = true) {
		...
		$elementIds = FabrikArray::getValue($filters, 'elementid', array());
		...
		$return = array();
		$eval = FabrikArray::getValue($filters, 'eval', array());
		$joins = FabrikArray::getValue($filters, 'join', array());
		$condition = FabrikArray::getValue($filters, 'condition', array());
		$groupedTo = FabrikArray::getValue($filters, 'grouped_to_previous', array());
		/*
		...
		if ($filterIndex === false || $normal) {
			...
			$condition = FabrikArray::getValue($condition, $filterIndex, $this->getFilterCondition());
			$eval = FabrikArray::getValue($eval, $filterIndex, FABRIKFILTER_TEXT);
		}

		$searchTypes = FabrikArray::getValue($filters, 'search_type', array());
		$searchType = FabrikArray::getValue($searchTypes, $filterIndex, 'normal');
		...
		$join = $searchType === 'prefilter' ? 'AND' : FabrikArray::getValue($joins, $filterIndex, 'AND');
		$groupedToPrevious = FabrikArray::getValue($groupedTo, $filterIndex, '0');
	...
	protected function getFilterType() {
		...
		$qsValues = FabrikArray::getValue($qsFilter, 'value', array());
	...
	public function getLabelForValue($v, $defaultLabel = null, $forceCheck = false) {
		...
		return ($key === false) ? $v : FabrikArray::getValue($labels, $key, $defaultLabel);
	}
	 ...
	 * Get a custom query
	 *
	 * @param   ModelList &$listModel list
	 * @param   string            $label      label
	 *
	 * @return  string
	 */
	...
	private function _median($results) {
		...
		if ((count($results) % 2) == 1) {
			...
			return $this->simpleAvg(array(FabrikArray::getValue($results, $midKey), FabrikArray::getValue($results, $midKey2)));
		}
	}
...
//		$safeHtmlFilter = InputFilter::getInstance(null, null, 1, 1);
	...
	public function getFieldDescription() {
		...
		$plugin = PluginHelper::getPlugin('fabrik_element', $element); // F5: May not be needed here, is already in provider
	...
	public function onStoreRow(&$data, $repeatCounter = 0) {
		...
		if (!array_key_exists($name, $formModel->formDataWithTableName)) {
			//if ($this->dataConsideredEmpty(FabrikArray::getValue($formModel->formDataWithTableName, $name, ''))) {
	...
	public function renderListData($data, \stdClass  &$thisRow, $opts = array()) {
		...
		foreach ($data as $i => &$d) {
			...
			if ((int) $params->get('icon_folder', 0) > 0 && FabrikArray::getValue($opts, 'icon', 1)) {
			...
			if (FabrikArray::getValue($opts, 'rollover', 1)) {
	...
	protected function getAddOptionFields($repeatCounter, $onlylabel = false) {
		...
		$displayData = new \stdClass;
		$displayData->id = $this->getHTMLId($repeatCounter);
		$displayData->add_image = FabrikHtml::image('plus', 'form', @$this->tmpl, array('alt' => Text::_('COM_FABRIK_ADD')));
	...
	public function getDefaultProperties($properties = array()) {
		...
		$formXml = new \SimpleXMLElement(file_get_contents(JPATH_ADMINISTRATOR.'/components/com_fabrik/forms/element.xml'));
		$now = $this->date->toSql();
		$this->setId(0);
		$item = $this->getElement();
		$formDefaults = FabrikWorker::formDefaults('element');
		$item->set('plugin', $this->_name);
		$item->set('params', json_encode((object)array_merge((array)json_decode($this->getDefaultAttribs()), $formDefaults['params'])));
	...
	public function getDefaultAttribs() {
		// Defaults are in the plugin.xml form
		/* set the elements default params */
		$form = $this->getPluginForm(0);
		$params = new \stdClass();
		foreach ($form->getXml()->xpath('//fieldset') as $xmlFieldSet) {
			foreach ($xmlFieldSet->xpath('//field') as $xmlField) {
				...
				if (empty($attributes['name']) || empty($attributes['default'])) {
					continue;		// We only want named fields with defaults
				}
				$params->{(string)($attributes['name'][0])} = (string)($attributes['default'][0]);
			}
		}
	...
	public function includeInSearchAll($advancedMode = false, $search = '') {
		...
		$params = json_decode(json_encode($listParams), true);

		// F4: list_search_elements is an object
		if(!array_key_exists('fabrik5', $params)) {
			$searchElements = json_decode(implode(",", $searchElements));
		...
		// F5: list_search_elements is an array
			if (in_array($this->getId(), $searchElements)) {
				$advancedMode = $listParams->get('search-mode-advanced');
				return $this->canIncludeInSearchAll($advancedMode);
			}
		}
	 ...
	 *
	 * @return  ModelJoin    join model
	 */
	public function getJoinModel() {
		if (is_null($this->joinModel)) {
			$this->joinModel = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Join', 'Site');
...
//			$join = FabTable::getInstance('Join', 'FabrikTable');
	...
	public function getJoinRepeatCount($data, $oJoin) {
		return count(FabrikArray::getValue($data, $oJoin->table_join . '___id', array()));
	}
	...
	protected function loadMeForAjax() {
		...
		$this->setId($input->getInt('element_id'));
		$this->list = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('List', 'Site');
	...
	public function onFinalStoreRow(&$data) {
		...
		if (array_key_exists('fabrik_copy_from_table', $formData)) {
			$idName = $name . '_id';
			$idValues = FabrikArray::getValue($formData, $idName, array());
		...
		$d = FabrikArray::getValue($formData, $rawName, FabrikArray::getValue($formData, $name));
		...
		if ($groupModel->isJoin()) {
			...
			$k = 'rowid';
			$parentIds = empty($allJoinValues) ? array() : FabrikArray::array_fill(0, count($allJoinValues), $formData[$k]);
		}

		$paramsKey = $this->getJoinParamsKey();
		$allParams = (array) FabrikArray::getValue($formData, $paramsKey, array());
		...
		foreach ($parentIds as $parentId) {
			...
			if ($groupModel->canRepeat()) {
				$joinValues = FabrikArray::getValue($allJoinValues, $i, array());
			} else {
			...
			foreach ($joinValues as $jIndex => $jid) {
				...
				$fkVal = FabrikArray::getValue($joinValues, $jIndex);
				$record->$shortName = $fkVal;
				$record->params = FabrikArray::getValue($allParams, $jIndex);
	...
	protected function swapValuesForLabels(&$d) {
		...
		foreach (array_keys($groups) as $gkey) {
			...
			for ($j = 0; $j < count($elementModels); $j++) {
				...
				$v = FabrikArray::getValue($d, $elKey);

				if (is_array($v)) {
					$origData = FabrikArray::getValue($d, $elKey, array());
					...
					foreach (array_keys($v) as $x) {
						$origVal = FabrikArray::getValue($origData, $x);
						$d[$elKey][$x] = $elementModel->getLabelForValue($v[$x], $origVal, true);
					}
				} else {
					$origData = FabrikArray::getValue($d, $elKey);
