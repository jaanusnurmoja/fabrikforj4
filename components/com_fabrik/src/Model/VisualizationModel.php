 ...
 * @since    3.0
 */
class VisualizationModel extends FabSiteModel {
	...
	public function getVisualization() {
		if (!isset($this->row)) {
//			$this->row = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createTable('Visualization', 'Administrator');
		...
		if (!isset($this->tables)) {
			...
			foreach ($this->listids as $id) {
				if (!array_key_exists($id, $this->tables)) {
					$listModel = BaseDatabaseModel::getInstance('List', 'Site');
					$listModel->setId($id);
					$listModel->getTable();
	...
	public function getFilters() {
		...
		foreach ($listModels as $listModel) {
			$show = (bool) FabrikArray::getValue($showFilters, $i, true);
	...
	public function getAdvancedSearchLink() {
		...
		$img = FabrikHtml::image('find', 'list', '', $opts);
	...
	public function setPrefilters() {
		...
		foreach ($listModels as &$listModel) {
			...
			$preFilter = FabrikArray::getValue($preFilters, $c);
			$preFilter = ArrayHelper::fromObject(json_decode($preFilter));
			$conditions = ArrayHelper::getValue($preFilter, 'filter-conditions', array(), 'array');
