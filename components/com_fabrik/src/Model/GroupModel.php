...
class GroupModel extends FabSiteModel {
	...
	public function &getGroup() {
		if (is_null($this->group)) {
			...
			$this->group = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createTable('Group', 'Administrator');
...
//		}
		...
		if ($widths !== '') {
			...
			$w = FabrikArray::getValue($widths, ($rowIx) % $colCount, $w);
		}
 ...
 */
	public function copy() {
		...
		$formGroup = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createTable('Formgroup', 'Administrator');
		...
		if (!$fkOnParent) {
			...
			for ($i = 0; $i < $repeats; $i++) {
				...
				if ($formModel->copyingRow()) {
					...
					$pk = $canRepeat ? FabrikArray::getValue($formData[$pkField], $i, '') : $formData[$pkField];
			...
			$pk = FabrikArray::getValue($formData, $fkField . '_raw', FabrikArray::getValue($formData, $fkField, ''));
 ...
 */
	protected function repeatTotals() {
		...
		return (int) FabrikArray::getValue($repeatTotals, $this->getGroup()->id, 0);
	}
 ...
 */
	public function getOrigGroupRowsIds() {
		$groupId = $this->getId();
		$formModel = $this->getFormModel();
		$origGroupRowsIds = FabrikArray::getValue($formModel->formData, 'fabrik_group_rowids', array());
		$origGroupRowsIds = FabrikArray::getValue($origGroupRowsIds, $groupId, '[]');
 ...
 */
	public function repeatCount() {
		...
		if (!empty($elementModels)) {
			...
			$d = FabrikArray::getValue($data, $smallerElHTMLName, array());
