 ...
 * @since       3.0
 */
class DetailsModel extends FabFormModel {
	/**
	 * id
	 * @var int
	 */
...
//		print_r("tmpl = ");print_r($tmpl);exit;// bootstrap
...
//		print_r("this->tmpl = ");print_r($this->tmpl);exit;// bootstrap
		return $this->tmpl;
	}
	...
	public function getFormCss() {
		...
		if ($tmpl != '') {
			 ...
			 * If FabrikHtml::styleSheetajax loaded then don't do &amp;
			 */
			$view = $this->isEditable() ? 'form' : 'details';

			// checks raw format (should have a more descriptive name like checkraw() )
			if (FabrikHtml::cssAsAsset()) {
				$qs .= '&view=' . $v;
				$qs .= '&rowid=' . $this->getRowId();
			} else {
				$qs .= '&amp;view=' . $v;
			...
			// For WAM we need an asset name. Create the asset name here and add this as param to FabrikHtml::stylesheetFromPath
			$tmplPath = 'templates/' . $this->app->getTemplate() . '/html/com_fabrik/' . $view . '/' . $tmpl . '/template_css.php' . $qs;
			$assetName = 'tem-' . substr($this->app->getTemplate(), 0, 3) . '-' . $view . '-' . substr($tmpl, 0, 3) . '-template_css';
//			if (!FabrikHtml::stylesheetFromPath($tmplPath))
			if (!FabrikHtml::stylesheetFromPath($tmplPath, $assetName)) {
				$assetName = 'com-' . $view . '-' . substr($tmpl, 0, 3) . '-template_css';
//				FabrikHtml::stylesheetFromPath('components/com_fabrik/tmpl/' . $view . '/' . $tmpl . '/template_css.php' . $qs);
				FabrikHtml::stylesheetFromPath('components/com_fabrik/tmpl/' . $view . '/' . $tmpl . '/template_css.php' . $qs, $assetName);
			}
			...
			$assetName = 'tem-' . substr($this->app->getTemplate(), 0, 3) . '-' . $view . '-' . substr($tmpl, 0, 3) . '-custom';
//			if (!FabrikHtml::stylesheetFromPath('templates/' . $this->app->getTemplate() . '/html/com_fabrik/' . $view . '/' . $tmpl . '/custom.css' . $qs))
			if (!FabrikHtml::stylesheetFromPath('templates/' . $this->app->getTemplate() . '/html/com_fabrik/' . $view . '/' . $tmpl . '/custom.css' . $qs, $assetName)) {
				$assetName = 'com-' . $view . '-' . substr($tmpl, 0, 3) . '-custom';
//				FabrikHtml::stylesheetFromPath('components/com_fabrik/tmpl/' . $view . '/' . $tmpl . '/custom.css' . $qs);
				FabrikHtml::stylesheetFromPath('components/com_fabrik/tmpl/' . $view . '/' . $tmpl . '/custom.css' . $qs, $assetName);
			}

			$path = 'templates/' . $this->app->getTemplate() . '/html/com_fabrik/' . $view . '/' . $tmpl . '/custom_css.php' . $qs;
			$assetName = 'tem-' . substr($this->app->getTemplate(), 0, 3) . '-' . $view . '-' . substr($tmpl, 0, 3) . '-custom_css';

//			if (!FabrikHtml::stylesheetFromPath($path))
			if (!FabrikHtml::stylesheetFromPath($path, $assetName)) {
				...
				$assetName = 'com-' . $view . '-' . substr($tmpl, 0, 3) . '-custom_css';
//				FabrikHtml::stylesheetFromPath($path);
		...
		if ($this->app->isClient('administrator') && $input->get('tmpl') === 'components') {
			FabrikHtml::stylesheet('administrator/templates/system/css/system.css');
		}
	}
 ...
 *
 * @return  ModelGroup[]  Group model objects with table row loaded
 */
	public function getGroups() {
		if (!isset($this->groups)) {
			...
			foreach ($groupData as $id => $groupD) {
				...
				$row = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createTable('Group', 'Administrator');
 ...
 *
 * @return  ModelGroup[]  Group & element objects
 */
 ...
 */
	public function process() {
		...
		$sessionModel = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Formsession', 'Site');
...
*/
		...
		if (isset($value) && isset($repeatCount) && is_array($value)) {
			$value = FabrikArray::getValue($value, $repeatCount, $default);
		}
 ...
 */
	public function &setFormData() {
		...
		if (!array_key_exists('__pk_val', $data)) {
			...
			$data['__pk_val'] = FabrikArray::getValue($data, $primaryKey . '_raw', FabrikArray::getValue($data, $primaryKey, ''));
		}
 ...
 */
	private function callElementPreProcess() {
		...
		foreach ($groups as $groupModel) {
			...
			$repeatedGroupCount = FabrikArray::getValue($repeatTotals, $group->id, 0, 'int');
			$elementModels = $groupModel->getPublishedElements();
 ...
 */
	protected function removeEmptyNoneJoinedGroupData(&$data) {
		$repeats = FabrikArray::getValue($data, 'fabrik_repeat_group', array());
 ...
 */
	protected function prepareForCopy() {
		...
		$origId = FabrikArray::getValue($this->formData, $k, '');
 ...
 *
 * @return  ModelList  fabrik list model
 */
	public function getListModel() {
		if (!isset($this->listModel)) {
			$this->listModel = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('List', 'Site');
			$item = $this->getForm();
			...
			foreach ($elementModels as $elementModel) {
				...
				if ($direction === 'toraw') {
					if (!array_key_exists($elName2Raw, $post) || $override) {
						...
						$input->set($elName2Raw, FabrikArray::getValue($post, $elName2, ''));
						$post[$elName2Raw] = FabrikArray::getValue($post, $elName2, '');
					}
				} else {
					if (!array_key_exists($elName2 . '_raw', $post) || $override) {
						// Post required getValue() later on
						$input->set($elName2, FabrikArray::getValue($post, $elName2Raw, ''));
						$post[$elName2] = FabrikArray::getValue($post, $elName2Raw, '');
					}
				}
			}
		}
 ...
 */
	public function validate() {
		...
		FabrikHtml::debug($this->errors, 'form:errors');
 ...
 */
	public function getTable($name = '', $prefix = '', $options = array()) {
		if (is_null($this->form)) {
			$this->form = parent::getTable('Form', 'Administrator');
		}
 ...
 */
	public function getElementIds($ignore = array(), $opts = array()) {
		...
		if (FabrikArray::getValue($opts, 'loadPrefilters', false)) {
			$listModel = $this->getListModel();
 ...
 */
	private function getElementIds_check($elementModel, $ignore, $opts, &$aEls) {
		...
		if (!in_array($class, $ignore)) {
			...
			if (!(FabrikArray::getValue($opts, 'includePublised', true) && $element->published == 0)) {
				$aEls[] = (int) $element->id;
			}
		}
	}
		...
		foreach ($groups as $gid => $groupModel) {
			...
			foreach ($elementModels as $elementModel) {
				...
				if ($key != 'id') {
					...
					if ($incRaw && is_a($elementModel, 'Fabrikar\Plugin\Fabrik_element\Field\Extension\Databasejoin')) {
 ...
 */
	public function getRowId() {
		...
		if ($this->getListModel()->getParams()->get('sef-slug', '') !== '' && !is_null($this->rowId)) {
 ...
 */
	...
	public function getData() {
		...
		if (!$form->record_in_database) {
			FabrikHtml::debug($data, 'form:getData from $_REQUEST');
			...
			if ($this->hasErrors()) {
				...
				if ($this->isMambot) {
					...
					if ($sessionRow->data != '') {
						...
						$data = array($data);
						FabrikHtml::debug($data, 'form:getData from session (form in Mambot and errors)');
					}
				} else {
					...
					FabrikHtml::debug($data, 'form:getData from POST (form not in Mambot and errors)');
				}
			} else {
				$sessionLoaded = false;
				...
				if ($this->saveMultiPage()) {
					...
					if ($sessionRow->data != '') {
						...
						$data = $bits;
						FabrikHtml::debug($data, 'form:getData from session (form not in Mambot and no errors');
					}
				}
				...
				if (!$sessionLoaded) {
					...
					if (!empty($useKey) || $this->rowId !== '') {
						...
						$fabrikDb->setQuery($sql);
						FabrikHtml::debug((string) $fabrikDb->getQuery(), 'form:render');
						...
						FabrikHtml::debug($data, 'form:getData from querying rowid= ' . $this->rowId . ' (form not in Mambot and no errors)');
...
//									JError::raiseWarning(500, Text::_('COM_FABRIK_COULD_NOT_FIND_RECORD_IN_DATABASE'));
		...
		$this->data = $data;
		FabrikHtml::debug($data, 'form:data');
 ...
 */
	public function setJoinData(&$data) {
		...
		$data = ArrayHelper::fromObject(FabrikArray::getValue($data, 0, $data));
	}
 ...
 */
	protected function getSessionData() {
		...
		$params = $this->getParams();
		$this->sessionModel = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Formsession', 'Site');
 ...
 */
	public function buildQuery($opts = array()) {
		...
		if (!$random && FabrikArray::getValue($opts, 'ignoreOrder', false) === false) {
			...
			if ($groupModel->isJoin()) {
				...
				foreach ($jData as $key => $array) {
					if ($groupModel->canRepeat()) {
						...
						$jData[$key] = FabrikArray::getValue($aVals, 0, '');
					}
				}
			}
		}
 ...
 */
	public function getRelatedTables() {
		...
		$listModel = $this->getListModel();
		$referringTable = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('List', 'Site');
		...
		foreach ($joinsToThisKey as $joinKey => $element) {
			...
			if ($val == '') {
				...
				if (empty($val)) {
					...
					$val = FabrikArray::getValue($this->data, $thisKey, $val);
			...
			if (isset($linkedLists->$key) && $linkedLists->$key != 0) {
				...
				$popUpLink = FabrikArray::getValue($linkedtable_linktype->$key, $f, false);
	...
	public function getGroupView($tmpl = '') {
		...
		foreach ($groups as $gkey => $groupModel) {
			...
			if (!empty($repeatGroups)) {
				$repeatGroup = FabrikArray::getValue($repeatGroups, $gkey, $repeatGroup);
 ...
 */
	public function applyMsgOnce() {
		...
		if (!empty($msg)) {
			$msg = FabrikArray::getValue($msg, 0);
 ...
 */
	public function getRedirectMessage() {
		...
		$customMsg = FabrikArray::getValue($sMsg, $customMsg);
