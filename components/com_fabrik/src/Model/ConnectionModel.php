...
class ConnectionModel extends FabSiteModel {
	 ...
	 *
	 * @return  Connection  Connection tables
	 */
	public function &getConnection($id = null) {
		...
		if (!is_object($this->connection)) {
			if ($this->id == -1 || $this->id == '' || $this->id == 0) {
				...
				$this->connection = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createTable('Connection', 'Administrator');
	...
	public function &loadDefaultConnection() {
		if (!$this->defaultConnection) {
			...
			$row = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createTable('Connection', 'Administrator');
