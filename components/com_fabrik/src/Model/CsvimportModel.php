 ...
 * @since       3.0
 */
class CsvimportModel extends FormModel {
	/**
	...
	protected function getFieldDelimiter() {
		...
		if (is_null($this->fieldDelimiter)) {
			...
			$tabDelimiter = FabrikArray::getValue($data, 'tabdelimited');
			$this->fieldDelimiter = $tabDelimiter == 1 ? "\t" : FabrikArray::getValue($data, 'field_delimiter', $this->fieldDelimiter);
	...
	public function readCSV($file) {
		...
		$text_delimiter = stripslashes(FabrikArray::getValue($data, 'text_delimiter', '"'));
		...
		$csv->inPutFormat = FabrikArray::getValue($data, 'inPutFormat', 'csv');
	...
	public function insertData() {
		...
		$dropData = (int) FabrikArray::getValue($jForm, 'drop_data', $dropData);

		$overWrite = FabrikWorker::getMenuOrRequestVar('csv_import_overwrite', '0', false, 'menu');
		$overWrite = (int) FabrikArray::getValue($jForm, 'overwrite', $overWrite);
...
*/
	...
	private function insertJoinedData($joinData) {
		...
		foreach ($joinData as $data) {
			...
			$js = FabrikArray::getValue($data, 'join', array());
	...
	private function _fakeJoinData($joinData, $aRow, $pkVal, &$formModel) {
		...
		if ($overWrite == '') {
			$overWrite = $app->getInput()->getInt('overwrite', 0, 'post');
		}
	...
	public function getSelectKey() {
		...
		$addKey = (int) FabrikArray::getValue($post, 'addkey', 0);
