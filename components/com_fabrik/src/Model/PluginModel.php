...
class PluginModel extends CMSPlugin implements SubscriberInterface {
	...
	public function __construct(&$subject, $config = array()) {
		...
		$this->_db = ArrayHelper::getValue($config, 'db', Factory::getDbo()); // Factory::getDbo() deprecated
		$this->config = ArrayHelper::getValue($config, 'config', Factory::getApplication()->getConfig());
		$this->user = ArrayHelper::getValue($config, 'user', Factory::getUser()); // Factory::getUser() deprecated
		...
		$this->date = ArrayHelper::getValue($config, 'date', Factory::getDate()); // Factory::getDate() deprecated
		$this->session = ArrayHelper::getValue($config, 'session', Factory::getSession()); // Factory::getSession() deprecated
	...
	{
		return [
			'onRenderAdminSettings' => 'onRenderAdminSettings',
		];
	}
	...
	protected function renderFromNavTabHeadings($form, &$output, $repeatCounter = 0) {
		...
		$layout = FabrikHtml::getLayout('fabrik-tabs');
		...
		if (empty($data['id'])) {
			...
			foreach ($form->getXml()->xpath('//fieldset') as $xmlFieldSet) {
				foreach ($xmlFieldSet->xpath('//field') as $xmlField) {
					...
					$data[(string)($attributes['name'][0])] = (string)($attributes['default'][0]);
				}
			}
		}
 ...
 */
	public function setParams(&$params, $repeatCounter) {
		...
		foreach ($opts as $key => $val) {
			if (is_array($val)) {
				$data[$key] = FabrikArray::getValue($val, $repeatCounter);
			} else {
				$data[$key] = $val;
			}
		}
	...
	public function ajax_fields() {
		...
		try
		{
			if ($showAll) {
				// Show all db columns
				$cid = $input->get('cid', -1);
				$cnn = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Connection', 'Site');
				$cnn->setId($cid);
				$db = $cnn->getDb();
...
//		$condition = $params->get($paramName);
		$formModel = $this->getModel(); // F5: already there or is this a different model ? the form model is NOT created here but plugin model
		$w = new FabrikWorker;
