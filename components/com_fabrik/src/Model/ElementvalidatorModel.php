		...
		foreach ($usedPlugins as $usedPlugin) {
			if ($usedPlugin !== '') {
				...
				if ($isPublished) {
					...
					PluginHelper::getPlugin('fabrik_validationrule', $usedPlugin); // F5: May not be needed here, is already in provider
					...
					$plugIn->getParams()->set('show_icon', FabrikArray::getValue($showIcon, $i, true));
					$plugIn->getParams()->set('validate_in', FabrikArray::getValue($validateIn, $i, 'both'));
					$plugIn->getParams()->set('validation_on', FabrikArray::getValue($validationOn, $i, 'both'));
					$plugIn->getParams()->set('must_validate', FabrikArray::getValue($mustValidate, $i, '0'));
					$plugIn->getParams()->set('validate_hidden', FabrikArray::getValue($validateHidden, $i, '1'));
					$plugIn->js();
					$c++;
				}
			}
...
//		return $j3 ? 'star.png' : 'notempty.png';
	...
	public function labelIcons() {
		...
		$icon = empty($emptyIcon) && empty($validations) ? "" : FabrikHtml::image($emptyIcon, 'form', $tmpl, $this->iconOpts) . ' ';
	...
	public function hoverTexts() {
		...
		if ($this->elementModel->isEditable()) {
			...
			if ($internal !== '') {
				...
				$icon = FabrikHtml::image($i, 'form', $tmpl, $this->iconOpts);
				$texts[] = $icon . ' ' . $internal;
			}
		}
