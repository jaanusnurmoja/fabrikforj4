 ...
 * @since       3.0
 */
class PluginlistModel extends PluginModel {
	/**
	 * Button prefix
	 *
	 * @var string
	...
	public function button(&$args) {
		$model = $this->getModel(); // function in PluginModel
	...
	public function button_result() {
		if ($this->canUse()) {
			...
			FabrikHtml::addPath('plugins/fabrik_list/' . $p . '/images/', 'image', 'list');
			$name = $this->_getButtonName();
			$label = $this->buttonLabel();
			$imageName = $this->getImageName();
			$tmpl = $this->getModel()->getTmpl(); // getModel() = function in PluginModel
			$properties = array();
			$opts = array(
				'forceImage' => false,
			);
			...
			$img = FabrikHtml::image($imageName, 'list', $tmpl, $properties, false, $opts);
			$text = $this->buttonAction == 'dropdown' ? $label : '<span class="hidden">' . $label . '</span>';

			if ($this->buttonAction != 'dropdown') {
				$layout = FabrikHtml::getLayout('fabrik-button');
