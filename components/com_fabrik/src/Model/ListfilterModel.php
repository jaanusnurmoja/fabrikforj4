...
class ListfilterModel extends FabSiteModel {
	 ...
	 * Set the list model
	 *
	 * @param   ModelList  $model  list model
	 *
	 * @return  void
	 */
...
//print_r("replacefilters?");exit;//yes
			$this->clearFilters();
		}
...
//print_r("incfilterss?");exit;//no
		...
		FabrikHtml::debug($filters, 'filter array: after querystring filters');
		$request = $this->getPostFilterArray();
		JDEBUG ? $profiler->mark('listfilter:request got') : null;
		$this->counter = count(FabrikArray::getValue($request, 'key', array()));
		...
		FabrikHtml::debug($filters, 'filter array: after session filters');
		JDEBUG ? $profiler->mark('listfilter:session filters got') : null;

		// The search form search all has lower priority than the filter search all and search form filters. F5: confusing explanation
		...
		FabrikHtml::debug($filters, 'filter array: search form');
		...
		FabrikHtml::debug($filters, 'filter array: after getpostfilters');
		$this->request = $filters;
		FabrikHtml::debug($this->request, 'filter array');
	...
	public function checkAccess(&$filters) {
		$access = FabrikArray::getValue($filters, 'access', array());
		...
		FabrikHtml::debug($filters, 'filter array: after access taken into account');
	}
	...
	private function getAdvancedSearchMode() {
		...
		if ($this->app->getInput()->get('option', '') === 'com_search') {
			...
			$mode = $this->app->getUserStateFromRequest(
				'com_fabrik.list' . $this->listModel->getRenderContext() . '.searchallmode',
	...
	public function getSearchAllValue($mode = 'html') {
		...
		if (trim($v) == '') {
			$fromFormId = $this->app->getUserState('com_fabrik.searchform.fromForm', '');

			if ($fromFormId != $this->listModel->getFormModel()->getForm()->id) {
				$v = $this->app->getUserState('com_fabrik.searchform.form' . $fromFormId . '.searchall', '');
			}
		}
	...
	private function getSearchAllFilters(&$filters) {
		$input = $this->app->getInput();
		$requestKey = $this->getSearchAllRequestKey();
		//print_r($requestKey);exit;// fabrik_list_filter_all_4_com_fabrik_4
		...
		if ($search == '') {
			if (array_key_exists($requestKey, $_POST)) {
				...
				$filterKeys = array_keys(FabrikArray::getValue($filters, 'search_type', array()));

				foreach ($filterKeys as $filterKey) {
					if (FabrikArray::getValue($filters['search_type'], $filterKey, '') == 'searchall') {
						foreach ($ks as $k) {
							/**
...
//print_r("or get here?");exit;//no
		...
		$searchTypes = FabrikArray::getValue($reg, 'search_type', array());
//print_r($searchTypes);exit;//Array ( [0] => searchall )

		for ($i = 0; $i < count($searchTypes); $i++) {
			if ($this->canClear($searchTypes[$i])) {
//print_r("canClear?");exit;//yes
				$this->clearAFilter($reg, $i);//ok
			}
		}
		...
		if (isset($registry->_registry) && isset($registry->_registry['com_fabrik']['data']->$tid->plugins)) {
//print_r("get registry?");exit;//no
...
//print_r("fromFormId?");exit;//yes
			$this->app->setUserState('com_fabrik.searchform.form' . $fromFormId . '.searchall', '');
		}
	}
	...
	protected function defaultAccessLevel() {
		...
		return FabrikArray::getValue($accessLevels, 0, 1);
	}
	...
	private function getSearchFormSearchAllFilters(&$filters) {
		...
		if ($fromFormId != $formModel->getId()) {
			$search = $this->app->getUserState('com_fabrik.searchform.form' . $fromFormId . '.searchall');
	...
	private function getSearchFormFilters(&$filters) {
		...
		if (!empty($fromFormId)) {
			...
			$lookupKeys = FabrikArray::getValue($filters, 'key', array());

			if ($fromFormId != $formModel->get('id')) {
				$fromForm = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Form', 'Site');
				...
				$searchFilters = $this->app->getUserState('com_fabrik.searchform.form' . $fromFormId . '.filters');
	...
	private function getQuerystringFilters(&$filters) {
		...
		foreach ($request as $key => $val) {
			...
			$eval = is_array($val) ? FabrikArray::getValue($val, 'eval', FABRIKFILTER_TEXT) : FABRIKFILTER_TEXT;
			$condition = is_array($val) ? FabrikArray::getValue($val, 'condition', $elementModel->getDefaultFilterCondition())
			...
			if (is_array($val)) {
				$value = FabrikArray::getValue($val, 'value', '');
				$join = FabrikArray::getValue($val, 'join', 'AND');
				$grouped = FabrikArray::getValue($val, 'grouped_to_previous', 0);
	...
	private function getPostFilters(&$filters) {
		...
		$elements = $this->listModel->getElements('id');
		$filterKeys = array_keys($filters);
		$values = FabrikArray::getValue($request, 'value', array());
		$searchTypes = FabrikArray::getValue($filters, 'search_type', array());
		$conditions = FabrikArray::getValue($request, 'condition', array());

		$usedMerges = array();

		FabrikHtml::debug($filters, 'filter array: start getPostFilters');

		if (!empty($request) && array_key_exists('key', $request)) {
			$keyInts = array_keys($request['key']);
			$ajaxPost = StringHelper::strtolower(is_null($input->server->get('HTTP_X_REQUESTED_WITH')) ? '' : $input->server->get('HTTP_X_REQUESTED_WITH'));
			...
			foreach ($keyInts as $i) {
				$value = FabrikArray::getValue($values, $i, '');
				$key = FabrikArray::getValue($request['key'], $i);
				$elid = FabrikArray::getValue($request['elementid'], $i);
				$condition = FabrikArray::getValue($conditions, $i);
				...
				if ($emptyValue && $index !== false) {
					 ...
					 */
					if (FabrikArray::getValue($searchTypes, $index) == 'normal') {
				...
				if ($index !== false) {
					foreach ($filterKeys as $fKey) {
						if (is_array($filters[$fKey]) && array_key_exists($index, $filters[$fKey])) {
							...
							if (FabrikArray::getValue($searchTypes, $index) == 'searchall' && $value == '') {
								continue 2;
							}
				...
				if (is_array($value)) {
					...
					if (FabrikArray::getValue($value, 0) == '' && FabrikArray::getValue($value, 1) == '') {
						continue;
					}
				}
				...
				if (!empty($filters)) {
					if ($i == 0) {
						$joinModes = FabrikArray::getValue($filters, 'join', array('AND'));
						$joinMode = array_pop($joinModes);

						// $$$ rob - If search all made, then the post filters should filter further the results
						$tmpSearchTypes = FabrikArray::getValue($filters, 'search_type', array('normal'));
				...
				$filters['search_type'][] = FabrikArray::getValue($request['search_type'], $i, 'normal');
				$filters['match'][] = $element->filter_exact_match;
				$filters['full_words_only'][] = $elParams->get('full_words_only');
				$filters['eval'][] = $eval;
				$filters['required'][] = $elParams->get('filter_required');
				$filters['access'][] = $elParams->get('filter_access');
				$filters['grouped_to_previous'][] = FabrikArray::getValue($request['grouped_to_previous'], $i, '0');
				$filters['label'][] = $elementModel->getListHeading();
		...
		FabrikHtml::debug($filters, 'filter array: before onGetPostFilter');
		FabrikWorker::getPluginManager()->runPlugins('onGetPostFilter', $this->listModel, 'list', $filters);
		FabrikHtml::debug($filters, 'filter array: after onGetPostFilter');
	...
	private function getSessionFilters(&$filters) {
		...
		$postKeys = FabrikArray::getValue($request, 'key', array());

		for ($i = 0; $i < count($sessionFilters['key']); $i++) {
			$elid = FabrikArray::getValue($sessionFilters['elementid'], $i);
			$key = FabrikArray::getValue($sessionFilters['key'], $i, null);
			$index = FabrikArray::getValue($filters['elementid'], $key, false);
			$origCondition = FabrikArray::getValue($filters['orig_condition'], $i, '');

			// Used by radius search plugin
			$sqlConds = FabrikArray::getValue($sessionFilters, 'sqlCond', array());
			...
			$key2 = array_key_exists('key2', $sessionFilters) ? FabrikArray::getValue($sessionFilters['key2'], $i, '') : '';
			...
			if ($elid == -1) {
				...
				if (in_array($elid, $pluginKeys)) {
					...
					$sqlCond = FabrikArray::getValue($sqlConds, $i);
