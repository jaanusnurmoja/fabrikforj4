 ...
 * @since       3.0
 */
class ForminlineeditModel extends FabFormModel {
	/**
	 * @var ModelForm
	 */
	protected $formModel;
...
FabrikHtml::script($srcs, implode("\n", $onLoad));
 */
	}
	...
	protected function inlineEditMarkUp() {
		...
		if ($input->getBool('inlinesave') || $input->getBool('inlinecancel')) {
			...
			if ($input->getBool('inlinecancel') == true) {
				...
				$html[] = FabrikHtml::image('delete.png', 'list', $thisTmpl, array('alt' => Text::_('COM_FABRIK_CANCEL')));
				$html[] = '<span>' . Text::_('COM_FABRIK_CANCEL') . '</span></a>';
			}

			if ($input->getBool('inlinesave') == true) {
				$html[] = '<a href="#" class="btn btn-primary inline-save">';
				$html[] = FabrikHtml::image('save.png', 'list', $thisTmpl, array('alt' => Text::_('COM_FABRIK_SAVE')));
