...
class ListModel extends FormModel {
	...
	public function __construct($config = array()) {
		...
		$this->setId($id);
		$this->advancedSearch = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('AdvancedSearch', 'Site');
	...
	public function render() {
		...
		FabrikHtml::debug($_POST, 'render:post');
		$input = $this->app->getInput();
	...
	public function &getFilterModel() {
		if (!isset($this->filterModel)) {
			$this->filterModel = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Listfilter', 'Site');
	...
	public function finesseData($opts = array()) {
		...
		FabrikHtml::debug((string) $fabrikDb->getQuery(), 'list GetData:' . $this->getTable()->label);
	...
	protected function formatData(&$data, $opts = array()) {
		...
		if ($this->outputFormat != 'pdf' && $this->outputFormat != 'csv' && $this->outputFormat != 'feed') {
			...
			FabrikHtml::debug($data, 'table:data');
		}
	...
	protected function addSelectBoxAndLinks(&$data, $opts = array()) {
		...
		if (!FabrikArray::getValue($opts, 'add_box_and_links', 1)) {
 ...
 */
	...
	public function getRecordCounts(&$element, $pks = array()) {
		...
		$listModel = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('List', 'Site');
		...
		FabrikHtml::debug($query->dump(), 'getRecordCounts query: ' . $linkKey);
		FabrikHtml::debug($this->recordCounts[$k], 'getRecordCounts data: ' . $linkKey);
	...
	protected function facetedTable($id) {
		...
		if (!array_key_exists($id, $this->facettables)) {
			$this->facettables[$id] = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('List', 'Site');
	...
	public function viewDataLink($popUp = false, $element = null, $row = null, $key = '', $val = '', $count = 0, $f = null) {
		...
		*/
		...
		if ($facetTable->canView()) {
			...
			$linkedForm = FabrikArray::getValue($existingLinkedForms, $f, false);
	 ...
	 * @param   PluginelementModel  &$elementModel  Element model
	...
	public function _addLink($data, &$elementModel, $row, $repeatCounter = 0) {
		...
		$html = FabrikHtml::loadDOMDocument($data);
	...
	public function linkHref($elementModel, $row, $repeatCounter = 0) {
		...
		if ($customLink == '') {
			...
			foreach ($array as $k => &$v) {
				...
				*/
				if (is_array($v)) {
					$v = FabrikArray::getValue($v, $repeatCounter);
				} else {
					...
					if ($v2 !== null) {
						if (is_array($v2)) {
							$v = FabrikArray::getValue($v2, $repeatCounter);
						} else {
							$v = $v2;
						}
					}
		...
		$layout = FabrikHtml::getLayout('element.fabrik-element-details-link');
		$displayData = new \stdClass;
		$displayData->row = $row;
...
//print_r("order_by = ");print_r($table->order_by);exit;
			...
			$by = trim($table->order_by) === '0' || trim($table->order_by) === '' ? array() : (array) json_decode($table->order_by); // F5: none = 0
			...
			$db->setQuery($query);
			FabrikHtml::debug((string) $query, 'table:mergeJoinedData get ids');
			$ids = array();
	...
	public function buildQueryOrder($query = false) {
		...
		if (!$userHasOrdered) {
			...
			if ($orderBys[0] == '' || $orderBys[0] == '0') { // F5: none = 0
			...
			if ($orderDirs[0] == '' || $orderDirs[0] == '-' || $orderDirs[0] == '0') {
			...
			if (!empty($orderBys)) {
				...
				foreach ($orderBys as $orderByRaw) {
					$dir = FabrikArray::getValue($orderDirs, $o, 'desc');
	 ...
	 *
	 * @param   PluginelementModel  $element  Element model
	 *
	 * @return  string
	 */
	public function buildQueryPrefilterWhere($element) {
		...
		$valueKeys = array_keys(FabrikArray::getValue($filters, 'value', array()));
	...
	private function groupFilterSQL(&$filters, $type = '*') {
		...
		$valueKeys = array_keys(FabrikArray::getValue($filters, 'key', array()));
		...
		foreach ($valueKeys as $vkey => $i) {
			...
			$condition = StringHelper::strtoupper(FabrikArray::getValue($filters['condition'], $i, ''));
			$searchType = $filters['search_type'][$i];
			if (FabrikArray::getValue($filters['sqlCond'], $i, '') == '' && !in_array($condition, $nullElementConditions)) {
				continue;
			}
			...
			if ($filters['origvalue'][$i] != '' || $filters['sqlCond'][$i] != '') {
				...
				$glue = FabrikArray::getValue($filters['join'], $i, 'AND');
	...
	public function getTable($name = '', $prefix = '', $options = array()) {
		...
		if (!isset($this->table) || !is_object($this->table)) {
			$this->table = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createTable('List', 'Administrator');
			$id = $this->getId();
				...
				if ($params->get('menu_access_only', '0') === '1') {
					...
					if (empty($itemId)) {
						...
						$query->select('id')
							...
							->where('link = "index.php?option=com_fabrik&view=list&listid=' . (int) $id . '"');
						$db->setQuery($query);
...
//		Table::addIncludePath(JPATH_ADMINISTRATOR . '/components/com_fabrik/table');
		$row = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createTable('List', 'Administrator');
//		$row = FabTable::getInstance('List', 'FabrikTable');
	...
	protected function getJoinsNoCdd() {
		if (!isset($this->_joinsNoCdd)) {
			$form = $this->getFormModel();
			$form->getGroupsHiarachy();
			$ignore = array('Cascadingdropdown');
	...
	public function shouldUpdateElement(&$elementModel, $origColName = null) {
		...
		*/
		...
		$thisFieldDesc = FabrikArray::getValue($dbDescriptions, $origColName, new \stdClass );
	 ...
	 *
	 * @return  ModelForm	form model with form table loaded
	 */
	public function &getFormModel() {
		if (!isset($this->formModel)) {
			$this->formModel = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Form', 'Site');
			$table = $this->getTable();
	...
	public function &getFilterArray() {
		...
		FabrikHtml::debug($request, 'filter:request');
		...
		*/
		...
		$indexStep = count(FabrikArray::getValue($this->filters, 'key', array()));
		FabrikHtml::debug($keys, 'filter:request keys');
		...
		FabrikHtml::debug($this->filters, 'listmodel::getFilterArray middle');
		$readOnlyValues = array();
		$w = new FabrikWorker;
		$noFiltersSetup = FabrikArray::getValue($this->filters, 'no-filter-setup', array());
		...
		$elementIds = FabrikArray::getValue($this->filters, 'elementid', array());
		$sqlCond = FabrikArray::getValue($this->filters, 'sqlCond', array());
		$raws = FabrikArray::getValue($this->filters, 'raw', array());
		...
		foreach ($this->filters['key'] as $i => $keyval) {
			...
			$elid = FabrikArray::getValue($elementIds, $i);
			$key2 = array_key_exists('key2', $this->filters) ? FabrikArray::getValue($this->filters['key2'], $i, '') : '';
			...
			*/
			$raw = FabrikArray::getValue($raws, $i, false);
		...
		FabrikHtml::debug($this->filters, 'end filters');
		...
		FabrikWorker::getPluginManager()->runPlugins('onFiltersGot', $this, 'list');
		FabrikHtml::debug($this->filters, 'after plugins:onFiltersGot');
		return $this->filters;
	}
	...
	private function showInList() {
		...
		$listElements = FabrikWorker::getMenuOrRequestVar('list_elements', '', $this->isMambot, 'menu', $opts);
		$listPrefilter = FabrikWorker::getMenuOrRequestVar('subform_prefilters', '', $this->isMambot, 'menu', $opts);
// OLD: "list_elements":"{\"show_in_list\":[\"75\",\"76\"]}" or "list_elements":"{\"show_in_list\":[]}" if empty
// NEW: "list_elements":["43","46"],
//print_r($listElements);exit;
// OLD:
// NEW: stdClass Object ( [subform_prefilters0] => stdClass Object ( [filter-join] => and [filter-fields] => 43 [filter-conditions] => equals [filter-value] => 03-05-2023 [filter-eval] => 1 [filter-access] => 1 ) [subform_prefilters1] => stdClass Object ( [filter-join] => or [filter-fields] => 46 [filter-conditions] => equals [filter-value] => 04-5-2023 [filter-eval] => 1 [filter-access] => 1 ) )
//print_r($listPrefilter);exit;
		// Need to convert multiselect array into object (old structure) or just don't json_decode?
//		$listElements = json_decode($listElements);
...
//$showInList = [42,43,46];
	...
	private function menuModulePrefilters() {
		...
		if ($requestRef !== '' && !strstr($requestRef, 'com_fabrik')) {
			...
			if (count($ref) > 1) {
				...
				if ($moduleId !== 0) {
					...
					// We have both the old prefilter object and the new prefilter subform array
					// On upgrade we need to add the prefilter subform array. This needs to have a different name: subform_prefilter
					// If no upgrade we need to create old prefilter object from the new prefilter subform array
		...
		if (!strstr($this->getRenderContext(), 'mod_fabrik_list') && $moduleId === 0) { // No module, so we get the menu params
			$spoof_check = array(
				'view' => 'list',
				'listid' => $this->getId(),
			);
			// F5: Need to check for "fabrik5" first
			$params = $this->getParams();
			$par = json_decode(json_encode($params), true);
			if(array_key_exists('fabrik5', (array)$par)) { // F5 way
				$properties = FabrikWorker::getMenuOrRequestVar('subform_prefilters', '', $this->isMambot, 'menu', $spoof_check);// F5
			} else {
				$properties = FabrikWorker::getMenuOrRequestVar('prefilters', '', $this->isMambot, 'menu', $spoof_check);// F4
				if (!FabrikWorker::isJSON($properties)) $properties = '';
			}
		}
		return $properties;
	}
	...
	public function prefilterSetting() {
		...
		$par = json_decode(json_encode($params), true);
		$listJoins = $listFields = $listConditions = $listValue = $listAccess = $listEval = $listGrouped = $listSearchType = [];
		if(array_key_exists('fabrik5', (array)$par)) { // F5 way
			if(array_key_exists('subform_prefilters', (array)$par) && $par['subform_prefilters'] != '') { 
				$p = (array) $par['subform_prefilters'];
				foreach ($p as $val) {
					$listJoins[] = $val['filter-join'];
					$listFields[] = $val['filter-fields'];
					$listConditions[] = $val['filter-conditions'];
					$listValue[] = $val['filter-value'];
					$listAccess[] = $val['filter-access'];
					$listEval[] = $val['filter-eval'];
					$listGrouped[] = $val['filter-grouped'];
				}
				$listSearchType = FabrikArray::array_fill(0, count($listJoins), 'prefilter');
			}
		} else {
			// F4 way
...
//print_r($properties);exit;
		if ($properties !== '') {
			if(array_key_exists('fabrik5', (array)$par)) { // F5 fabrik5 check also done in menuModulePrefilters
				$prefilters = json_decode(json_encode($properties), true);// F5 and yes we have a condition
//print_r($prefilters);exit;
				foreach ($prefilters as $val) {
					$joins[] = $val['filter-join'];
					$fields[] = $val['filter-fields'];
					$conditions[] = $val['filter-conditions'];
					$values[] = $val['filter-value'];
					$access[] = $val['filter-access'];
					$eval[] = $val['filter-eval'];
				}
//print_r($fields);exit;
				if (!empty($conditions)) $searchType = FabrikArray::array_fill(0, count($joins), 'menuPrefilter');
			} else { // F4
				$prefilters = ArrayHelper::fromObject(json_decode($properties));// F4
				$fields = FabrikArray::getValue($prefilters, 'filter-fields', array());
				$conditions = FabrikArray::getValue($prefilters, 'filter-conditions', array());
				$values = FabrikArray::getValue($prefilters, 'filter-value', array());
				$access = FabrikArray::getValue($prefilters, 'filter-access', array());
				$eval = FabrikArray::getValue($prefilters, 'filter-eval', array());
				$joins = FabrikArray::getValue($prefilters, 'filter-join', array());
				if (!empty($conditions)) $searchType = FabrikArray::array_fill(0, count($joins), 'menuPrefilter');
			}
			if (!empty($conditions)) {
				...
				if ($overrideListPrefilters) {
					...
					$joins[0] = 'AND';
					$listFields = array_merge((array)$listFields, $fields);
					$listConditions = array_merge((array)$listConditions, $conditions);
					$listValue = array_merge((array)$listValue, $values);
					$listAccess = array_merge((array)$listAccess, $access);
					$listEval = array_merge((array)$listEval, $eval);
					$listSearchType = array_merge((array)$listSearchType, $searchType);
					//$listGrouped[count($listGrouped) -1] = '1';
					$listJoins = array_merge((array)$listJoins, $joins);

					$listGrouped = array_merge((array)$listGrouped, FabrikArray::array_fill(0, count($joins), 0));
				}
			}
			
		}
//print_r($listFields);exit;
 ...
 */
	public function getPrefilterArray(&$filters) {
		if (!isset($this->prefilters)) {
			...
			if(!empty($filterFields)) {
				for ($i = 0; $i < count($filterFields); $i++) {
					...
					$join = FabrikArray::getValue($filterJoins, $i, 'AND');
					...
					$selValue = FabrikArray::getValue($filterValues, $i, '');
					$filterEval = FabrikArray::getValue($filterEvals, $i, false);
					$filterGrouped = FabrikArray::getValue($filterGroupeds, $i, false);
					...
					$elementModel = FabrikArray::getValue($elements, FabrikString::safeColName($tmpFilter), false);
		...
		FabrikHtml::debug($filters, 'prefilters');
	}
 ...
 */
	protected function getJoinMergeTotalRecords() {
		...
		FabrikHtml::debug($totalSql, 'table getJoinMergeTotalRecords');
		$total = $db->loadResult();
 ...
 *
 * @return  ModelGroup[]	element objects
 */
 ...
 */
	public function &getPagination($total = 0, $limitStart = 0, $limit = 0) {
		if (!isset($this->nav)) {
			...
			$params = $this->getParams();
			$this->nav = new FabrikPagination($total, $limitStart, $limit);
 ...
 */
	public function gotOptionalFilters() {
		$filters = $this->getFilterArray();
		$types = FabrikArray::getValue($filters, 'search_type', array());
...
// F5: Why is this &makeFilters and not makeFilters
		$aFilters = array();
		$fScript = array();
		$opts = new \stdClass;
		...
		// search-all filter only: We (temporary) want to exclude date & jdate from search-all
		if (($params->get('search-mode', '0') == '1')
			...
			if ($params->get('search-mode-advanced') == 1) {
				...
				$displayData->mode = $this->app->getUserStateFromRequest(
					'com_fabrik.list' . $this->getRenderContext() . '.searchallmode',
				...
				if (isset($element->filter_type) && $element->filter_type != '' && $element->filter_type != 'null') {
					if ($elementModel->canView('list') && $elementModel->canUseFilter()) {
						...
						*/
						...
						if(!in_array($plugin, $plugins)) { // F5: prevent double loading
		...
		// F5: only need to do if we have any filters
		if ($params->get('search-mode', '0') || $params->get('advanced-filter', '0') || ($params->get('show-table-filters', '0') && $counter > 0)) {
...
// Check for search form filters - if they exists create hidden elements for them F5: not clear what thay mean to do here
		$keys = FabrikArray::getValue($filters, 'key', array());
		...
		foreach ($keys as $i => $key) {
			if ($filters['no-filter-setup'][$i] == '1' && !in_array($filters['search_type'][$i], array('searchall', 'advanced', 'jpluginfilters'))) {
				...
				*/
				...
				$o->filter = FabrikArray::getValue($filters['filter'], $i);
	...
	public function getHeadings() {
		...
		// F5: Responsive element classes & oai
		if(array_key_exists('fabrik5', (array)$params)) { // Most likely always true, but just in case
			if(array_key_exists('subform_list_responsive_elements', (array)$params)) { 
				$p = (array) $params['subform_list_responsive_elements'];
				$par = json_decode(json_encode($p), true);
				foreach ($par as $val) {
					$tmp['responsive_elements'][] = $val['responsive_elements'];
					$tmp['responsive_class'][] = $val['responsive_class'];
				}
				$listClasses = (object) $tmp;
			}
			if(array_key_exists('subform_open_archive_elements', (array)$params)) { 
				$p = (array) $params['subform_open_archive_elements'];
				$par = json_decode(json_encode($p), true);
				foreach ($par as $val) {
					$tmp['dublin_core_element'][] = $val['dublin_core_element'];
					$tmp['dublin_core_type'][] = $val['dublin_core_type'];
					$tmp['raw'][] = $val['raw'];
				}
				$oai = (object)$tmp;
			}
		} else { // Old F4 way.
			if ($p = (array) $params['list_responsive_elements']) {
				$par = json_decode(json_encode($p), true);
				foreach ($par as $val) {
					$tmp['responsive_elements'][] = $val['responsive_elements'];
					$tmp['responsive_class'][] = $val['responsive_class'];
			...
			if ($p = (array) $params['open_archive_elements']) {
				$par = json_decode(json_encode($p), true);
				foreach ($par as $val) {
					$tmp['dublin_core_element'][] = $val['dublin_core_element'];
					$tmp['dublin_core_type'][] = $val['dublin_core_type'];
		...
		foreach ($groups as $groupModel) {
			...
			foreach ($elementModels as $key => $elementModel) {
				...
				if ($elementParams->get('can_order') == '1' && $this->outputFormat != 'csv') {
					$context = 'com_fabrik.list' . $this->getRenderContext() . '.order.' . $elementId;
					...
					if (is_null($orderDir)) {
						...
						if ($ix !== false) {
							$orderDir = FabrikArray::getValue($orderDirs, $ix, '');
						} else {
				...
				$responsiveClass = $responsiveKey !== false ? FabrikArray::getValue($listClasses->responsive_class, $responsiveKey, '') : '';
		...
		if (!empty($pluginHeadings) || $this->canSelectRows() || $this->canEditARow() || $details || $edit || $filtersUnderHeadingsAndGo) {
			...
			if (empty($headingButtons) || $hidecheckbox == '1' || !$this->canSelectRows()) {
				...
				if ($this->actionMethod() == 'dropdown') {
					$align = $params->get('checkboxLocation', 'end') == 'end' ? 'right' : 'left';
					$aTableHeadings['fabrik_actions'] = FabrikHtml::bootStrapDropDown($headingButtons, $align);
				} else {
					$aTableHeadings['fabrik_actions'] = FabrikHtml::bootStrapButtonGroup($headingButtons);
				}
			}
	...
	public function storeRow($data, $rowId, $isJoin = false, $joinGroupTable = null) {
						...
						if ($elementModel->recordInDatabase($data)) {
							if (array_key_exists($key, $data) && !in_array($key, $noRepeatFields)) {
								...
								if ($elementModel->isJoin() && $isJoin && array_key_exists('params', $data)) {
									...
									$oRecord->params = FabrikArray::getValue($data, 'params');
		...
		*/
		...
		if (!$ok) {
			...
			// Clean the cache.
			Factory::getCache('com_fabrik')->clean();
	...
	public function updateObject($table, &$object, $keyName, $updateNulls = true) {
		...
		FabrikHtml::debug((string) $db->getQuery(), 'list model updateObject:');
	...
	public function insertObject($table, &$object, $keyName = null) {
		...
		FabrikHtml::debug((string) $db->getQuery(), 'list model insertObject:');
				...
				if (($isJoin && $groupModel->isJoin()) || (!$isJoin && !$groupModel->isJoin())) {
					...
					foreach ($elementModels as $elementModel) {
						...
						if (!$elementModel->canUse()) {
							...
							$default = array();
							$repeatGroupCount = FabrikArray::getValue($repeatGroupCounts, $groupModel->getGroup()->id);
					...
					foreach ($elementModels as $elementModel) {
						...
						if ($element->name == $key) {
							...
							if (!$elementModel->canUse()) {
								...
								for ($repeatCount = 0; $repeatCount < $repeatGroupCount; $repeatCount++) {
									$enc = FabrikArray::getValue($encrypted, $repeatCount);
	...
	public function addIndex($field, $prefix = '', $type = 'INDEX', $size = '') {
		...
		*/
		...
		FabrikArray::filter($indexes, 'Column_name', $field);
	...
	public function getRSSFeedLink() {
		...
		if ($this->getParams()->get('rss') == '1') {
			...
			$link = $base . '?option=com_fabrik&view=list&listid=' . $this->getId();
			$link .= "&format=feed";
	...
	private function replaceWithRowData($matches) {
		...
		$return = FabrikArray::getValue($this->aRow, $match);
	...
	public function viewDetailsLink(&$row, $view = null) {
		...
		if (trim($customLink) === '') {
			...
			if ($this->app->isClient('administrator')) {
				...
				$link .= 'index.php?option=com_fabrik&view=' . $view . '&formid=' . $table->form_id . $keyIdentifier . '&Itemid=' . $itemId;
			}
	...
	public function getRow($id, $format = false, $loadJoin = false) {
		...
		if (!$loadJoin) {
			if ($format == true) {
				...
				*/
				$row = FabrikArray::getValue($row, FabrikArray::firstKey($row), array());
				$this->rows[$sig] = FabrikArray::getValue($row, 0, new \stdClass );
			} else {
			...
			if ($format == true) {
				...
				*/
				$this->rows[$sig] = FabrikArray::getValue($rows, FabrikArray::firstKey($rows), array());
			} else {
	...
	public function xRecord($mode = 'table') {
		...
		if ($mode == 'table') {
			...
			$this->app->redirect('index.php?option=com_fabrik&view=form&formid=' . $formId . '&rowid=' . $rowId . '&format=raw');
		}
	...
	public static function columnData($listId, $col, $distinct = true, $opts = array()) {
		...
		$listModel = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('List', 'Site');
		$listModel->setId($listId);
		$listModel->filters = FabrikArray::getValue($opts, 'filters');
		...
		$filterLimit = FabrikArray::getValue($opts, 'filterLimit', true);
		$where = FabrikArray::getValue($opts, 'where', '');
	 ...
	 *
	 * @return  PluginelementModel[]	matched element models
	 */
	 ...
	 *
	 * @return  PluginelementModel[]	table element models
	 */
	...
	public function getAddRecordLink() {
		...
		$keys = FabrikArray::getValue($filters, 'key', array());
		$vals = FabrikArray::getValue($filters, 'value', array());
		$types = FabrikArray::getValue($filters, 'search_type', array());

		for ($i = 0; $i < count($keys); $i++) {
			$filterType = FabrikArray::getValue($types, $i, '');
	...
	protected function formatForJoins(&$data) {
		...
		$formModel = $this->getFormModel();
		$db = $this->getDb();
		FabrikHtml::debug($data, 'render:before formatForJoins');
	...
	public function getListCss() {
		...
		if ($tmpl != '') {
			...
			$assetName = 'tem-' . substr($this->app->getTemplate(), 0, 3) . '-list-' . substr($tmpl, 0, 3) . '-template_css';

//			if (!FabrikHtml::stylesheetFromPath($overRide))
			if (!FabrikHtml::stylesheetFromPath($overRide, $assetName)) {
				$assetName = 'com-list-' . substr($tmpl, 0, 3) . '-template_css';
//				FabrikHtml::stylesheetFromPath('components/com_fabrik/tmpl/list/'. $tmpl . '/template_css.php' . $qs);
				FabrikHtml::stylesheetFromPath('components/com_fabrik/tmpl/list/' . $tmpl . '/template_css.php' . $qs, $assetName);
			}
			...
			*/
			$assetName = 'tem-' . substr($this->app->getTemplate(), 0, 3) . '-list-' . substr($tmpl, 0, 3) . '-custom';
//			if (!FabrikHtml::stylesheetFromPath('templates/' . $this->app->getTemplate() . '/html/com_fabrik/list/' . $tmpl . '/custom.css' . $qs))
			if (!FabrikHtml::stylesheetFromPath('templates/' . $this->app->getTemplate() . '/html/com_fabrik/list/' . $tmpl . '/custom.css' . $qs, $assetName)) {
				$assetName = 'com-list-' . substr($tmpl, 0, 3) . '-custom';
//				FabrikHtml::stylesheetFromPath('components/com_fabrik/views/list/' . $jTmplFolder . '/' . $tmpl . '/custom.css');
				FabrikHtml::stylesheetFromPath('components/com_fabrik/tmpl/list/' . $tmpl . '/custom.css', $assetName);
			}
			$assetName = 'tem-' . substr($this->app->getTemplate(), 0, 3) . '-list-' . substr($tmpl, 0, 3) . '-custom_css';
//			if (!FabrikHtml::stylesheetFromPath('templates/' . $this->app->getTemplate() . '/html/com_fabrik/list/' . $tmpl . '/custom_css.php' . $qs))
			if (!FabrikHtml::stylesheetFromPath('templates/' . $this->app->getTemplate() . '/html/com_fabrik/list/' . $tmpl . '/custom_css.php' . $qs, $assetName)) {
				$displayData = new \stdClass;
				...
				$assetName = 'com-list-' . substr($tmpl, 0, 3) . '-custom_css';
//				FabrikHtml::stylesheetFromPath($path);
				FabrikHtml::stylesheetFromPath($path, $assetName);
			}
		}
	}
	...
	public function getCsvFields() {
		...
		if ($params->get('csv_which_elements', 'selected') == 'visible') {
			...
			if(property_exists(json_decode($params), 'fabrik5')) {
				$csvIds = $params->get('csv_elements');
			} else {
	...
	private function getTabCategories() {
		...
		$isDropdown = (is_subclass_of($elementModel, 'Dropdown') || get_class($elementModel) == 'Dropdown');
		$is_join = (is_subclass_of($elementModel, 'Databasejoin') || get_class($elementModel) == 'Databasejoin');
		if (!$is_join) {
		 ...
		 **/
		$db->setQuery($query);
		FabrikHtml::debug($query->dump(), 'list getTabCategories query:' . $table->label);
		$profiler = Profiler::getInstance('Application');
		...
		FabrikHtml::debug($counts, 'list getTabCategories counts: ' . $table->label);
		...
		FabrikHtml::debug($counts, 'list getTabCategories merged counts: ' . $table->label);
	...
	public function loadTabs() {
		...
		$ActiveTabLimitStart = FabrikArray::getValue($inputArray, 'limitstart' . $listId, $uri->getVar('limitstart' . $listId, null));
		$ActiveTabLimit = FabrikArray::getValue($inputArray, 'limit' . $listId, $uri->getVar('limit' . $listId, null));
...
//		$context = 'com_'.$package.$menu.$item->id.'list'.$listId;
		...
		if (is_null($uriActiveTab)) {
			$uriActiveTab = FabrikArray::firstKey($cachedTabs);
		}
	...
	public function getLayout($name, $paths = array(), $options = array()) {
		...
		$layout = FabrikHtml::getLayout($name, $paths, $options);
