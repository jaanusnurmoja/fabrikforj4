 ...
 * @since       3.0
 */
class PluginformModel extends PluginModel {
	/**
	...
	public function getProcessData() {
		...
		$model = $this->getModel('Pluginform', 'Site'); // function in PluginModel
//		$model = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Plugin', 'Administrator');
		...
		foreach ($groups as $gKey => $groupModel) {
				...
				if ($groupModel->isJoin()) {
					...
					if (is_object($joinTable)) {
						...
						if (!empty($elementModels)) {
							...
							$tmpEl = FabrikArray::getValue($model->formDataWithTableName, $smallerElHTMLName, array(), 'array');
			...
			for ($c = 0; $c < $repeatGroup; $c++) {
				...
				foreach ($elementModels as $elementModel) {
					...
					if ($groupModel->isJoin()) {
						if ($groupModel->canRepeat()) {
							$raw = FabrikArray::getValue($model->formDataWithTableName[$k], $c, '');
						...
						$rawValue = FabrikArray::getValue($model->formDataWithTableName, $k . '_raw', '');
	...
	public function formJavascriptClass() {
		$formModel = $this->getModel(); // function in PluginModel
	...
	protected function doLog($msgType, $msg) {
		if ($this->log === null) {
//			$this->log = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createTable('Log', 'Administrator');
