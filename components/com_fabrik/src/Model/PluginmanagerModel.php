...
class PluginmanagerModel extends FabSiteModel {
	...
	protected function &loadPlugInGroup($group) {
		...
		$plugins = PluginHelper::getPlugin($folder); // F5: May not be needed here, is already in provider
	...
	public function getFormPlugins(&$form) {
		...
		if (!array_key_exists($sig, $this->formPlugins)) {
			...
			foreach ($elements as $element) {
				...
				require_once JPATH_PLUGINS . '/fabrik_element/' . $element->plugin . '/src/Extension/' . StringHelper::ucfirst($element->plugin) . '.php';
				$class = StringHelper::ucfirst($element->plugin);
...
//				}
	...
	public function getPluginFromId($id, $type = 'Element') {
		$el = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createTable($type, 'Administrator');
