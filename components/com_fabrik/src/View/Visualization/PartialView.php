 ...
 * @since       3.4.3
 */
class PartialView extends FabrikView
{
	/**
	 * Display
	 *
	...
	public function display($tmpl = 'default')
	{
		...
		$model = $this->getModel();
//		$model = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Visualization', 'Site');
...
isClient('administrator') ? JPATH_ADMINISTRATOR : JPATH_SITE;
		...
		{
			HTMLHelper::stylesheet('template.css', 'plugins/fabrik_visualization/' . $plugin->_name . '/tmpl/' . $tmpl . '/', true);// F5: add 2nd paran asset name
		}
