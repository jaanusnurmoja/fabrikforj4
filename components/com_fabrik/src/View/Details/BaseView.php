 ...
 * @since       3.0.6
 */
class BaseView extends FabrikView {
	/**
...
//		$model = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Form', 'Site');
	...
	private function _repeatGroupButtons($tmpl) {
		$formModel = $this->getModel('details'); // do they mean the form model ?
	...
	public function display($tpl = null) {
		...
		$tmpl = $model->getTmpl(); // bootstrap

		$this->tmpl = $tmpl;

		$this->_addButtons(); // F5: ok
...
//		print_r("this->data = ");print_r($this->data);exit;// ok correct data is there

		FabrikHtml::debug($this->groups, 'form:view:groups');

		$this->setTmplFolders($tmpl); //ok
		$this->_addJavascript($listModel->getId()); // F5: Change the name as this is not only JS
...
//		$model = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Form', 'Site');
		...
		$form->formid = 'details_' . $model->getId();
	...
	private function setTmplFolders($tmpl) {
		...
		$this->_basePath = COM_FABRIK_FRONTEND; // not used in this class, somewhere else ??
		$model = $this->getModel();
//		$model = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Form', 'Site');
		$jTmplFolder = 'tmpl'; // not used in this class, somewhere else ??
		$folder = 'details';
		$this->addTemplatePath(COM_FABRIK_FRONTEND . '/tmpl/details/' . $tmpl);

		$root = $this->app->isClient('administrator') ? JPATH_ADMINISTRATOR : JPATH_SITE;
		//$this->addTemplatePath($root . '/templates/' . $this->app->getTemplate() . '/html/com_fabrik/details/' . $tmpl);
	}
...
//		$model = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Form', 'Site');
		...
		if ($params->get('process-jplugins', 2) == 1
			...
			FabrikHtml::runContentPlugins($text, $cloak);
		}
...
//		$model = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Form', 'Site');
		...
		if ($input->get('tmpl') != 'component') {
			if ($this->showEmail) {
				$this->emailLink = FabrikHtml::emailIcon($model, $params);
				$this->emailURL = FabrikHtml::emailURL($model);
			}
		}
		...
		if ($input->get('print', 0) != 1) {
			...
			if ($this->showPrint) {
				$this->printLink = FabrikHtml::printIcon($model, $params);
				$this->printURL = FabrikHtml::printURL($model);
			}
		}
		...
		if ($this->showPDF) {
			if (FabrikWorker::canPdf(false)) {
				if ($this->app->isClient('administrator')) {
					// F5: ok details controller display is called
					$this->pdfURL = 'index.php?option=com_fabrik&task=details.view&format=pdf&formid=' . $model->getId() . '&rowid=' . $model->getRowId();
				} else {
					// F5: there is no task defined and there is no details view, so how to get pdf output?? will get error view not found
					$this->pdfURL = 'index.php?option=com_fabrik&view=details&formid=' . $model->getId() . '&rowid=' . $model->getRowId() . '&format=pdf';
				}

				$this->pdfURL = Route::_($this->pdfURL);
				$layout = FabrikHtml::getLayout('form.fabrik-pdf-icon');
	...
	protected function _addJavascript($listId) {
		...
		$model = $this->getModel();
//		$model = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Form', 'Site');
		$model->elementJsJLayouts(); // in turn this calls jsJLayout() for every element in pluginelementmodel which does nothing
		$params = $model->getParams();
...
//print_r("params = ");print_r($params);exit;// ok, is form params
//print_r("bKey = ");print_r($bKey);exit;// ok

		// ok, just loading js & css here
//		$wa->useScript('fabrik');//media/com_fabrik/js/fabrik.js
//		$wa->useScript('form');//media/com_fabrik/js/form.js
//		$wa->useScript('tipsBootStrapMock');//media/com_fabrik/js/tipsBootStrapMock.js

		$groups = $model->getGroupsHiarachy();
//print_r("groups = ");print_r($groups);exit;// endless
...
//		$liveSiteReq[] = $mediaFolder . '/tipsBootStrapMock.js';
		...
		if (!defined('_JOS_FABRIK_FORMJS_INCLUDED')) {
			...
			FabrikHtml::slimbox();
			// ok, just loading js & css here
//			$wa->useScript('form-submit');//media/com_fabrik/js/form-submit.js
//			$wa->useScript('element');//media/com_fabrik/js/element.js
//			$wa->useScript('elementlist');//media/com_fabrik/js/elementlist.js
//			$wa->useScript('form_placeholder');//media/com_fabrik/js/lib/form_placeholder/Form.Placeholder.js

			if ($tablesorter) {
//				$wa->useScript('tablesorter_combined'); //media/com_fabrik/js/lib/tablesorter/jquery.tablesorter.combined.js
//				$wa->useStyle('theme_blue');//media/com_fabrik/js/lib/tablesorter/theme.blue.min.css
			}
		}
		...
		foreach ($groups as $groupModel) {
			$groupParams = $groupModel->getParams(); // already did this on line 553

			if ($groupParams->get('repeat_sortable', '0') === '2') {
				if (!$tablesorter) {
//					$wa->useScript('tablesorter'); //media/com_fabrik/js/lib/tablesorter/jquery.tablesorter.min.js
//					$wa->useScript('tablesorter_widgets'); //media/com_fabrik/js/lib/tablesorter/jquery.tablesorter.widgets.min.js';
//					$wa->useStyle('theme_blue');//media/com_fabrik/js/lib/tablesorter/theme.blue.min.css
				}
			}
...
// Not needed when using WAM, but we need to load element JS file somewhere
...
//				}
				...
				// Load javascript code added in element
		...
		FabrikHtml::windows('a.fabrikWin');
		FabrikHtml::tips('.hasTip', array(), "$('$bKey')");
...
// element JS and element validation not yet working in F5, so temporary disable
/*
...
//		$model = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Form', 'Site');
		...
		$this->errorIcon = FabrikHtml::image($errorIcon, 'form', $this->tmpl);

		$imgs = new \stdClass;
		$imgs->alert = FabrikHtml::image($errorIcon, 'form', $this->tmpl, '', true);
		$imgs->action_check = FabrikHtml::image('action_check.png', 'form', $this->tmpl, '', true);

		$imgs->ajax_loader = FabrikHtml::icon('icon-spinner icon-spin');
		$opts->images = $imgs;
	...
	protected function _addJavascriptSumbit(&$script, $listId, $aWYSIWYGNames) {
		...
		$script[] = "\t\tdocument.location = '" . Route::_('index.php?option=com_fabrik&task=viewTable&cid=' . $listId) . "';";
		$script[] = "\t}";
...
//		$model = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Form', 'Site');
		...
		$fields[] = '<input type="hidden" name="option" value="com_fabrik" />';
		...
		$fields[] = '<input type="hidden" name="package" value="fabrik" />';
		...
		if ($useKey = FabrikWorker::getMenuOrRequestVar('usekey', '')) {
			...
			$pk_val = FabrikArray::getValue($model->data, $listModel->getPrimaryKey(true));
		...
		$btnLayout = FabrikHtml::getLayout('fabrik-button');

		if ($resetIcon !== '') {
			$resetIcon = FabrikHtml::icon($resetIcon);
		...
		if ($copyIcon !== '') {
			$copyIcon = FabrikHtml::icon($copyIcon);
		...
		if ($applyIcon !== '') {
			$applyIcon = FabrikHtml::icon($applyIcon);
		...
		if ($deleteIcon !== '') {
			$deleteIcon = FabrikHtml::icon($deleteIcon);
		...
		if ($goBackIcon !== '') {
			$goBackIcon = FabrikHtml::icon($goBackIcon);
		...
		if ($model->isEditable() && $params->get('submit_button', 1)) {
			...
			if ($submitIcon !== '') {
				$submitIcon = FabrikHtml::icon($submitIcon);
		...
		if ($this->isMultiPage) {
			$layoutData = (object) array(
				...
				'label' => FabrikHtml::icon('icon-previous', Text::_('COM_FABRIK_PREV')),
				'formModel' => $model,
			);
			...
			$layoutData = (object) array(
				...
				'name' => 'fabrikPageNext',
				'label' => Text::_('COM_FABRIK_NEXT') . '&nbsp;' . FabrikHtml::icon('icon-next'),
				'formModel' => $model,
			);
		...
		foreach ($groups as $groupModel) {
			if ($groupModel->isJoin()) {
				...
				if (array_key_exists($groupId, $origRowIds)) {
					...
					$groupPk .= '_raw';
					$groupRowIds = (array) FabrikArray::getValue($this->data, $groupPk, array());
		...
		foreach ($get as $key => $input) {
			...
			if ($formModel->hasElement($key)) {
				...
				if (!$elementModel->canUse()) {
					...
					if (is_array($input)) {
						$input = FabrikArray::getValue($input, 'raw', $input);
					}
