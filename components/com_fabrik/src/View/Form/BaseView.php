...
//		$model = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Form', 'Site');
	...
	private function setTmplFolders($tmpl) 
	{
		...
		$this->addTemplatePath(COM_FABRIK_FRONTEND . '/tmpl/' . $folder . '/' . $tmpl);
	...
	protected function setTitle($w, &$params) 
	{
		...
		if (!$this->app->isClient('administrator')) {
			...
			if (!$this->isMambot) {
				$titleData = array_merge($_REQUEST, $model->getData());
	...
	protected function _addButtons() 
	{
		...
		if ($this->showPDF) {
			// ok, for test we need to set this to true, just to show the button. PDF will be done later
//			if (FabrikWorker::canPdf(false))
	...
	protected function _addJavascript($listId) 
	{
		...
		$model = $this->getModel();
		$model->elementJsJLayouts(); // in turn this calls in pluginelementmodel jsJLayout() for every plugin which overloads jsJLayout(), but only databasejoin is doing that
		$params = $model->getParams();
...
//		if (!defined('_JOS_FABRIK_FORMJS_INCLUDED')) { // F5: Load only once. Not needed, because we use WAM
//			define('_JOS_FABRIK_FORMJS_INCLUDED', 1);
			FabrikHtml::slimbox(); // loading some js files
			//media/com_fabrik/js/elementlist.js // extends element, only for some elements: checkbox, link, radiobutton => module
		...
		foreach ($groups as $groupModel) {
			...
			foreach ($elementModels as $elementModel) {
				...
				for ($c = 0; $c < $eventMax; $c++) {
					...
					if (!empty($jsAct)) {
						$jsActions[] = $jsAct; // inline code generated.
					}
				}
			}
		}
		...
		foreach ($groups as $groupModel) {
			...
			foreach ($elementModels as $elementModel)
			{
				...
				if (($elementModel->canUse() && $model->isEditable()) || $elementModel->canView())
				{
					for ($c = 0; $c < $max; $c++)
					{
						$ref = $elementModel->elementJavascript($c); // function elementJavascript in PluginelementModel is overloaded by the element plugin
...
//		}
		...
		// Placeholder
//		$script[] = "\tnew Form.Placeholder('.fabrikForm input');"; // F5: See line 522: don't use
		$this->_addJavascriptSumbit($script, $listId, $aWYSIWYGNames);

//		if (FabrikHtml::inAjaxLoadedPage())
//		{
		// F5: FloatingTips is in tips.js and in tipsBootStrapMock.js, which do we use?? Or can we just use standard Joomla Bootstrap popovers?
		// F5: tipsBootStrapMock: Enable us to use the same class interface for tips.js but use Bootstrap popovers (Joomla 3)
...
//		}
		...
		// TODO: load custom form_XX files and Form plugin JS files by WAM
//		$str = implode("\n", $script);
//		$model->getCustomJsAction($srcs);// $srcs is an array with file names to load

		$pluginManager->runPlugins('onAfterJSLoad', $model);

//		$srcs = array_merge($srcs, $model->formPluginShim); 
		// formPluginShim is a property in the model. defined by function formJavascriptClass() in PluginformModel
		// $srcs is an array with file names to load
...
//		$str .= implode(',', $plugins) . "});\n";
	...
	protected function jsOpts() 
	{
		...
		// F5: Update all images to font awesome (default joomla)
...
//		$model = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Form', 'Site');
		$listModel = $model->getListModel();
		$row = ArrayHelper::toObject($model->getData());
