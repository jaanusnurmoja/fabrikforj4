...
class BaseView extends FabrikView
{
	...
	protected function csvJS(&$opts, $model)
	{
		...
		$exportUrl = 'index.php?option=com_fabrik&view=list&listid=' . $model->getId();
		...
		FabrikHtml::jLayoutJs('exportcsv', 'fabrik-modal', (object) $modalOpts);
	}
	...
	protected function getManagementJS($data = array())
	{
		...
		$doGroupby			= (bool) $params->get('do_group_by', false);
		...
		if ($ajaxLinks)
		{
			...
			FabrikHtml::jLayoutJs('ajax_links', 'fabrik-modal', (object) $modalOpts);
		}
		...
		if ($params->get('advanced-filter'))
		{
			...
			FabrikHtml::jLayoutJs('advanced-filter', 'fabrik-modal', (object) $modalOpts);
		}

		FabrikHtml::jLayoutJs('modal-state-label', 'list.fabrik-filters-modal-state-label', $layoutData = (object) array(
			'label' => '',
			'displayValue' => '',
			'key' => ''
		));
		...
		if($toggleCols) $wa->useScript('list-toggle'); // Class.
		if($doGroupby) $wa->useScript('list-grouped');// Class.
...
//print_r($src_1);exit; // Need to test
		...
		$pluginManager->runPlugins('loadJavascriptClassName', $model, 'list'); // No idea what this does.....
		...
		$model->getCustomJsAction($src_2[]); // more custom JS files to add
//print_r($src_2);exit; // Need to test
		// F5 TODO: load custom JS files in $src_2 by WAM
		// $wa->registerAndUseScript('bar', 'com_foobar/bar.js', [], [], ['core', 'foobar']);
		...
		{
			$wa->registerAndUseScript('fabcustom', $aJsPath);
		}
		...
		if (File::exists(JPATH_ROOT . '/components/com_fabrik/tmpl/list/' . $tmpl . '/default_empty_row.php'))
		{
		...
		$opts->itemTemplate = ob_get_contents(); // Store the contents of an output buffer in a variable
		ob_end_clean();
...
//print_r($script);exit; // Need to test
	...
	{
		// F5: layout is set in controller. input from button href is layout = '_advancedsearch'
		// F5: modal window already created
		// F5: display switches to advancedSearch window
		...
		$tmpl            = $model->getTmpl();
		$this->addTemplatePath(COM_FABRIK_FRONTEND. '/tmpl/' . $this->_name . '/' . $tmpl);
		...
		$this->showGroupBy	  = (bool) $params->get('do_group_by', false);
		...
		{
			$this->csvImportLink = 'index.php?option=com_fabrik&view=import&filetype=csv&listid=' . $item->id;
		...
		else
		{
			...
			$pdfLink = 'index.php?option=com_fabrik&Itemid=' . $itemId . '&view=list&format=pdf&listid=' . $item->id;
		...
		$this->tablePicker = $params->get('show-table-picker', $input->get('list-picker', true)) && $this->app->isClient('administrator') && $this->app->getInput()->get('format') !== 'pdf' ? FabrikHtml::tableList($this->table->id) : '';
	...
	protected function advancedSearch($tpl)
	{
		...
		$this->tmpl = $model->getTmpl(); // bootstrap
...
//			$model = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('List', 'Site');
			$id    = $model->getId();
			$url   = Route::_('index.php?option=com_fabrik&view=list&listid=' . $id);
		}
