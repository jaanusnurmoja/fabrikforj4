...
case 'desc':
	...
	$img = FabrikHtml::image('fa fa-sort-up', 'list', $d->tmpl, $imgProperties);
	break;
case 'asc':
	$d->orderDir = 'desc';
	$d->class = 'class="fabrikorder-asc"';
	$imgProperties['alt'] = Text::_('COM_FABRIK_ORDER');
	$img = FabrikHtml::image('fa fa-sort-down', 'list', $d->tmpl, $imgProperties);
	break;
case '':
case '-':
	$d->orderDir = 'asc';
	...
	$imgProperties['alt'] = Text::_('COM_FABRIK_ORDER');
	$img = FabrikHtml::image('sort', 'list', $d->tmpl, $imgProperties);
	break;
}
...
if ($d->class === '') {
	if (in_array($d->key, $d->orderBys)) {
		if ($d->item->order_dir === 'desc') {
			$d->class = 'class="fabrikorder-desc"';
			$imgProperties['alt'] = Text::_('COM_FABRIK_ORDER');
			$img = FabrikHtml::image('arrow-up.png', 'list', $d->tmpl, $imgProperties);
		}
	}
}

if ($d->elementParams->get('can_order', false)) {
	$heading = '<a ' . $d->class . ' ' . FabrikHtml::propertiesFromArray($headingProperties) . ' href="#">' . $img . $d->label . '</a>';
} else {
