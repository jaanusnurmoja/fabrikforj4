...
class ListfieldsField extends ListField
{
	...
	protected function getInput()
	{
		...
		if (is_array($res))
		{
			$isSubform = strpos($this->form->getName(), 'subform.') === 0;
			...
			{
				if ($useInput) {
					$formSelect = $isSubform ? 'form-select' : 'form-select-sm';
					$return = HTMLHelper::_('select.genericlist', $aEls, $this->name, 'class="' . $formSelect . ' inputbox" size="1" ', 'value', 'text', $this->value, $this->id);
				} else {
				...
				$return .= '<img style="margin-left:10px;display:none" id="' . $this->id
					. '_loader" src="/media/fabrik/com_fabrik/images/admin/ajax-loader.gif" alt="' . Text::_('LOADING') . '" />';
			}
		}
	...
	private function _listOptions($controller, $valueFormat, $useStep, $onlyListFields, $showRaw, $pluginFilters, $labelMethod, $noJoins)
	{
		...
		if (($id = $this->form->getValue('id')) === null) {
			$id = $app->input->get('id');;
		}
		...
		if (!isset($this->form->model))
		{
			switch ($controller) {
				case 'module':
					...
					$modParams = json_decode($db->loadResult());
                    $id = $modParams->list_id;
                    break;
				case 'item':
					$menuParams = json_decode($app->getMenu('site')->getItem($id)->getParams());
