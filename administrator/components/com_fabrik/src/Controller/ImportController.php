 ...
 * @since       3.0
 */
class ImportController extends FabFormController {
	/**
	...
	protected function addElements($model, $headings) {
		...
		$listModel = $this->getModel('List', 'Site');
		$listModel->setId($input->getInt('listid'));
		$item = $listModel->getTable();

		/** @var ModelList $adminListModel */
		$adminListModel = $this->getModel('List', '');
		...
		foreach ($newElements as $elName => $add) {
			if ($add) {
				...
				$element->label = FabrikString::strtolower($elName);
	...
	public function makeTableFromCSV() {
		...
		/** @var ModelCsvimport $model */
		$model = $this->getModel('Csvimport', 'Site');
	...
	public function display($cachable = false, $urlparams = array()) {
		...
		$this->getModel('Csvimport', 'Site')->clearSession();
	...
	public function doimport() {
		...
		if (!empty($model->newHeadings)) {
			$view->setModel($model, true);
			$view->setModel($this->getModel('Pluginmanager', 'Site'));
