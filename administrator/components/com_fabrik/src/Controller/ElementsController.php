 ...
 * @since       3.0
 */
class ElementsController extends FabAdminController {
	/**
	 ...
	 * @return  J model
	 */
	public function getModel($name = 'Element', $prefix = '', $config = array()) {
		$config = array();
	...
	public function toggleInList() {
		...
		$task = $this->getTask();
		$value = FabrikArray::getValue($data, $task, 0, 'int');

		if (empty($cid)) {
			\Joomla\CMS\Factory::getApplication()->enqueueMessage(Text::_($this->text_prefix . '_NO_ITEM_SELECTED'), 'warning');} else {
			// Get the model.
			$model = $this->getModel();
