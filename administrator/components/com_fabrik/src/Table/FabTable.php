...
class FabTable extends Table {
	...
	public static function getInstance($type, $prefix = 'Table', $config = array()) {
		$config['dbo'] = FabrikWorker::getDbo(true); // If true is used, than this is the joomla database
		$instance = parent::getInstance($type, $prefix, $config);
//			print_r("config['dbo'] = ");print_r($config['dbo']);exit;
	...
	public function batch($batch) {
		$batchParams = FabrikArray::getValue($batch, 'params');
		unset($batch['params']);
	...
	public function getFields($reload = false) {
		...
		if (FabrikArray::getValue($cache, $this->_tbl) === null || $reload) {
