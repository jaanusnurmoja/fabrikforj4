...
use Fabrikar\Component\Fabrik\Administrator\Model\FabListModel;
		...
		foreach ($items as &$item) {
			if ($item->full_element_name == '') {
				if ($item->db_table_name == '') {
					...
					$db->setQuery($query);
					$item->db_table_name = 'form' . $db->loadResult();
				}
			...
			$addAccessTitle = FabrikArray::getValue($viewLevels, $item->access);
			$addAccessTitle = is_object($addAccessTitle) ? $addAccessTitle->title : 'n/a';

			$editAccessTitle = FabrikArray::getValue($viewLevels, $params->get('edit_access', 1));
			$editAccessTitle = is_object($editAccessTitle) ? $editAccessTitle->title : 'n/a';

			$viewAccessTitle = FabrikArray::getValue($viewLevels, $params->get('view_access', 1));
			...
			if (is_object($validations) && property_exists($validations, 'plugin')) {
				for ($i = 0; $i < count($validations->plugin); $i++) {
					...
					. '<br />&nbsp;&nbsp;&nbsp;&nbsp;' . Text::_('COM_FABRIK_FIELD_ERROR_MSG_LABEL') . ': <em>' . htmlspecialchars(FabrikArray::getValue($msgs, $i, 'n/a')) . '</em>';
				}
			}
