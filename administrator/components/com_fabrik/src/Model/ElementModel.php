...
class ElementModel extends FabAdminModel {
	 ...
	 */
	public function getTable($type = 'Element', $prefix = '', $config = array()) {
		$config['dbo'] = FabrikWorker::getDbo(true);
	 ...
	 * @param   string $group   Name of the plugin group.
	 *
 ...
 */
	public function getPluginHTML($plugin = null) {
		...
		$input = $app->input;
		$str = '';
		$item = $this->getItem(); // database table row
	...
	public function validate($form, $data, $group = null) {
		...
		if (!$input->get('unlink', false) && (int) $data['id'] === 0) {
			...
			if ($joinTblId === 0) {
				...
				/** @var ModelList $joinListModel */
				$joinListModel = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('List', 'Site');
...
//print_r("data = ");print_r($data);exit;// yes, but how do we get this in pluginelementmodel ?
		...
		$row = $elementModel->getElement(); // store in db
	...
	public function copy() {
		...
		foreach ($cid as $id => $groupid) {
			$rule->load((int) $id);
			$name = FabrikArray::getValue($names, $id, $rule->name);
	...
	public function createRepeatElement($elementModel, $row) {
		...
		$field = FabrikArray::getValue($fields, $row->name, false);
	...
	private function getValidations($elementModel, $usedPlugins = array()) {
		...
		foreach ($usedPlugins as $usedPlugin) {
			if ($usedPlugin !== '') {
				$class = 'plgFabrik_Validationrule' . FabrikString::ucfirst($usedPlugin);
				$conf = array();
				$conf['name'] = FabrikString::strtolower($usedPlugin);
				$conf['type'] = FabrikString::strtolower('fabrik_Validationrule');
				$plugIn = new $class($dispatcher, $conf);
				$oPlugin = PluginHelper::getPlugin('fabrik_validationrule', $usedPlugin); // F5: May not be needed here, is already in provider
