...
class FormModel extends FabAdminModel {
	...
	public function getTable($type = 'Form', $prefix = '', $config = array()) {
 ...
 */
	public function save($data) {
		...
		$tmpName = FabrikArray::getValue($data, 'db_table_name');
		...
		if (FabrikWorker::isNullDate($row->get('publish_down'))) {
			$row->set('publish_down', $db->getNullDate());
		}
...
*/		
		...
		$currentGroups = (array) FabrikArray::getValue($data, 'current_groups');
		if ($isNew && !$recordInDatabase) {
			// we need to create the group, this is normally done when the form is created by the list process
			$groupData = FabrikWorker::formDefaults('group');
			$groupData['name'] = $dbTableName;
			$groupData['label'] = $dbTableName;

			$groupId = $listModel->createLinkedGroup($groupData, false, false);
			$currentGroups[] = $groupId;
			/* Create the default fields */
			$pluginManager = FabrikWorker::getPluginManager();
			$ordering = 0;
			foreach ($fields as $name => $plugin) {
				$elementModel = $pluginManager->getPlugIn($plugin, 'element');
				$element = $elementModel->getDefaultProperties();
				$element->set('group_id', $groupId);
	...
	private function getInsertFields($isNew, $data, $listModel, $dbTableName) {
		$db = FabrikWorker::getDbo(true);
		$fields = array('id' => 'internalid', 'date_time' => 'jdate');
		$createGroup = $data['_createGroup'];
		$recordInDatabase = $data['record_in_database'];
		$jForm = $this->app->input->get('jform', array(), 'array');
		$groups = FabrikArray::getValue($data, 'current_groups');
