...
class GroupModel extends FabAdminModel {
	 ...
	 */
	public function getTable($type = 'Group', $prefix = '', $config = array()) {
	...
	protected function joinedGroupExists($id) {
		$item = $this->getTable('Group', 'Administrator');
		$item->load($id);
	...
	protected function makeFormGroup($data) {
		...
		$id = (int) $data['id'];
		$item = $this->getTable('Formgroup', 'Administrator');
	...
	private function checkFKIndex($data) {
		...
		$pkField = FabrikArray::getValue($fields, $join->get('table_key'), false);
	...
	public function makeJoinedGroup(&$data) {
		...
		foreach ($elements as $element) {
			...
			if (!array_key_exists($fName, $names)) {
				...
				$field = FabrikArray::getValue($fields, $fName);
...
//		$elementModel = BaseDatabaseModel::getInstance('Element', 'FabrikModel');
		$elementModel = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Element', 'Administrator'); // F5 does it work now ??
	...
	public function deleteElements($pks) {
		...
		$db->setQuery($query);
		$elementIds = $db->loadColumn();
		$elementModel = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Element', 'Administrator');
