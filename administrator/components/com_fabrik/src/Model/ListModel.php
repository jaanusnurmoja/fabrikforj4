 ...
 * @since       3.0
 */
class ListModel extends FabAdminModel {
	/**
	 ...
	 */
	public function getTable($type = 'List', $prefix = 'Table', $config = array()) {
		$sig = $type . $prefix . implode('.', $config);

		if (!array_key_exists($sig, $this->tables)) {
			$config['dbo'] = FabrikWorker::getDbo(true);
			if (!$this->tables[$sig] = $this->_createTable($type, $prefix, $config)) {
 ...
 */
	 ...
	 *
	 * @return  ModelForm  front end form model
	 */
	public function getFormModel() {
		if (is_null($this->formModel)) {
			$config = array();
			$config['dbo'] = FabrikWorker::getDbo(true);
			$this->formModel = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Form', 'Site', $config);
	 ...
	 * We should no longer have these kind of solutions. Just copy "some of its methods" here
	 *
	 * @return  object  front end list model
	 */
	public function getFEModel() {
		if (is_null($this->feListModel)) {
			$this->feListModel = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('List', 'Site');
	...
	public function validate($form, $data, $group = null) {
		...
		if (empty($data['_database_name']) && FabrikArray::getValue($data, 'db_table_name') == '') {
	...
	public function save($data) {
		...
		$session->set('sess_id', $id);
		$session->set('sess_table_name', $data['db_table_name']);
		$session->set('sess_cid', $data['connection_id']);

		// Check for existing table
		if (is_null($id) || $id == 0) {
			// NULL maybe 0, but just in case...
			$isNew = false;
			// If we have a valid name in 'db_table_name', we have an existingTable and newTable is empty
			$existingTable = ArrayHelper::getValue($data, 'db_table_name', '');
			// If we have no valid name in 'db_table_name', newTable gets the value of '_database_name' and existingTable is empty
			$newTable = $existingTable === '' ? trim(FabrikArray::getValue($data, '_database_name')) : '';
		...
		$params = new Registry($listRow->get('params'));
		$isView = $this->setIsView($params);
		$data['params']['isview'] = (string) $isView;

		$this->setState('list.id', $id);
		$this->setState('list.form_id', $listRow->get('form_id'));
		$feModel = $this->getFEModel();

		// Get original collation
		$db = $feModel->getDb();
		$origCollation = $this->getOriginalCollation($params, $db, FabrikArray::getValue($data, 'db_table_name', ''));
		$listRow->bind($data); // $data was changed

		// F5: Copy order_by & order_dir values from subform params.
		if(array_key_exists('fabrik5', (array)$data['params'])) { // Will always be the case, but just for sure
			$orderings = $data['params']['subform_ordering'];
			foreach($orderings as $ordering) {
				$order_by[] = $ordering['order_by'];
				$order_dir[] = $ordering['order_dir'];
			}
			$listRow->set('order_by', json_encode($order_by));
			$listRow->set('order_dir', json_encode($order_dir));
		} else {
			$listRow->set('order_by', json_encode($input->get('order_by', array(), 'array')));
			$listRow->set('order_dir', json_encode($input->get('order_dir', array(), 'array')));
		}
		...
		if ($listRow->id != 0) {
			$this->collation($feModel, $origCollation, $listRow);
			if (FabrikWorker::isNullDate($listRow->get('created'))) {
				$listRow->set('created', $date->toSql());
			}
		}

		if (is_null($id) || $id == 0) {
			if ($listRow->get('created', '') == '') {
				$listRow->set('created', $date->toSql());
				$listRow->set('created_by', $this->user->get('id'));
				$listRow->set('created_by_alias', $this->user->get('username'));
			}

			$listRow->set('modified', $date->toSql());
			$listRow->set('modified_by', $this->user->get('id'));
			// Save the list row now
			$listRow->store();

			// Create fabrik form
			$this->createLinkedForm(); // check into: $data can be from store or xml. this is a new form

			// Set the form_id to store in list
			$listRow->set('form_id', $this->getState('list.form_id'));
			$params = new Registry($listRow->get('params'));
			...
			$listRow->params = $params->toString();
			...
			if ($newTable == '') {
				...
				$groupData['name'] = $listRow->label;
				$groupData['label'] = $listRow->label;
				$groupId = $this->createLinkedGroup($groupData, false); // check into: $data can not be from store. this is edit

				$this->createLinkedElements($groupId); // check into: $data can not be from store. this is edit
			} else {
				// New fabrik list and new table
				$listRow->set('db_table_name', $newTable);
				$listRow->set('auto_inc', 1);
				...
				$fields = [];
				if (PluginHelper::getPlugin('fabrik_element', 'internalid')) { // F5: May not be needed here, is already in provider
					$fields['id'] = 'internalid';
				} else {
					throw new \RuntimeException(Text::_('COM_FABRIK_PLUGIN_NOT_ENABLED'));
				}

				if (!empty($jForm['element_plgs'][0])) {
					foreach ($jForm['element_plgs'] as $plg_name) {
						if (PluginHelper::getPlugin('fabrik_element', $plg_name)) { // F5: May not be needed here, is already in provider
							$plg_name == 'jdate' ? $fields['date_time'] = $plg_name : $fields[$plg_name] = $plg_name;
							// Maybe add other column names here
							// Also need to add element params somewhere
						} else {
							throw new \RuntimeException(Text::_('COM_FABRIK_PLUGIN_NOT_ENABLED'));
						}
					}
			...
			$groupId = $this->createLinkedGroup($groupData, $isJoin, $isRepeat); // check into: $data can be from store or xml. this is a new group

			// Finally create the new table and add element column names
			$res = $this->createDBTable($newTable, $fields, $dbOpts); // check into: $data can be from store or xml. this is a new table

			if (is_array($res)) {
				// If db table was created
				$listRow->set('db_primary_key', $newTable . '.' . $res[0]);
			}
		}
		// Set the publish date
		if (FabrikWorker::isNullDate($listRow->get('publish_up'))) {
			if ($listRow->get('published') == 1) {
				$listRow->set('publish_up', Factory::getDate()->toSql());
			} else {
				$listRow->set('publish_up', $db->getNullDate());
			}
		}

		if (FabrikWorker::isNullDate($listRow->get('publish_down'))) {
			$listRow->set('publish_down', $db->getNullDate());
		}

		$pk = FabrikArray::getValue($data, 'db_primary_key');

		if ($pk == '') {
			$pk = $feModel->getPrimaryKeyAndExtra($listRow->get('db_table_name'));
			$key = $pk[0]['colname'];
			$extra = $pk[0]['extra'];

			// Store without qns as that's db specific
			$listRow->set('db_primary_key', $listRow->get('db_primary_key', '') == '' ? $listRow->get('db_table_name') . '.' . $key
				: $listRow->get('db_primary_key'));
			$listRow->set('auto_inc', StringHelper::stristr($extra, 'auto_increment') ? 1 : 0);
		}
		$listRow->set('created_by', (int) $listRow->get('created_by'));
		$listRow->set('checked_out', 0);
		//$listRow->set('checked_out_time', $db->getNullDate());
		$listRow->set('hits', 0);
		$listRow->set('private', 0);
		//$listRow->set('group_by', '');
		$listRow->store(); // Store $listRow data in new #__fabrik_list record

		$this->updateJoins($data);

		// Needed to ensure pk field is not quoted
		$feModel->setTable($listRow);

		if (!$feModel->isView()) {
			$this->updatePrimaryKey($listRow->get('db_primary_key'), $listRow->get('auto_inc'));
		}

		// Make an array of elements and a presumed index size, map is then used in creating indexes
		$this->createIndexes($params, $listRow);
		$pkName = $listRow->getKeyName();

		if (isset($listRow->$pkName)) {
			$this->setState($this->getName() . '.id', $listRow->get($pkName));
		}
	...
	public function setIsView($params) {
		...
		$feModel = $this->getFEModel(); // don't use a function, add code. don't use site ListModel
	...
	protected function createIndexes($params, $row) {
		...
		// F5: param group_by_order changed to subform: changed to array incl order dir
		if (trim($params->get('subform_group_by_ordering')->group_by_order) !== '') {
			$feListModel->addIndex($params->get('subform_group_by_ordering')->group_by_order, 'groupbyorder', 'INDEX', $map[$params->get('subform_group_by_ordering')->group_by_order]);
		}
	 ...
	 *
	 * @param   ModelList $feModel       Front end list model
	...
	protected function collation($feModel, $origCollation, $row) {
		...
		$newCollation = $params->get('collation'); // should be utf8mb4_general_ci
	 ...
	 * Does NOT check if table name exists in #__fabrik_lists !
	 *
...
// need a different error reporting here
	...
	private function updateJoins($data) {
		...
		$joinsToIndex = array();
		$joinModel = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Join', 'Site');

		if(array_key_exists('fabrik5', (array)$params)) { // F5 way
			if(array_key_exists('subform_joins', (array)$params) && $params['subform_joins'] != '') { 
				$p = (array) $par['subform_joins'];
				foreach ($p as $val) {
					$joinIds[] = $val['join_id'];
					$joinTypes[] = $val['join_type'];
					$joinTableFrom[] = $val['join_from_table'];
					$joinTable[] = $val['table_join'];
					$tableKey[] = $val['table_key'];
					$joinTableKey[] = $val['table_join_key'];
					$repeats[] = $val['join_repeat'];
				}
			} 
		} else {
			$joinIds = FabrikArray::getValue($params, 'join_id', array());
			$joinTypes = FabrikArray::getValue($params, 'join_type', array());
			$joinTableFrom = FabrikArray::getValue($params, 'join_from_table', array());
			$joinTable = FabrikArray::getValue($params, 'table_join', array());
			$tableKey = FabrikArray::getValue($params, 'table_key', array());
			$joinTableKey = FabrikArray::getValue($params, 'table_join_key', array());
			$repeats = FabrikArray::getValue($params, 'join_repeat', array());
		}
		if($joinTypes) $jc = count($joinTypes);
		...
		foreach ($joinsToIndex as $thisJoin) {
			...
			$fkField = FabrikArray::getValue($fields, $thisJoin->table_join_key, false);
	...
	protected function makeNewJoin($tableKey, $joinTableKey, $joinType, $joinTable, $joinTableFrom, $isRepeat) {
		...
		if ($pk !== false) {
			...
			$pk_col = FabrikArray::getValue($pk[0], 'colname', '');
	 ...
	 * column type
	 * This is the 'copy element' feature and should be in admin ElementModel
	 *
	...
	private function createLinkedElements($groupId, $tableName = '') {
		...
		if ($tableName === '') {
			$jForm = $input->get('jform', array(), 'array');
			$tableName = FabrikArray::getValue($jForm, 'db_table_name');
		}

		$pluginManager = FabrikWorker::getPluginManager();
		$groupTable = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createTable('Group', 'Administrator');
//		$groupTable    = FabTable::getInstance('Group', 'Table');
		$groupTable->load($groupId);

		// Here we're importing directly from the database schema
		$query = $db->getQuery(true);
		$query->select('id')->from('#__fabrik_lists')->where('db_table_name = ' . $db->q($tableName)); // need to add connection_id?
		$db->setQuery($query);
		$id = $db->loadResult();

		if ($id) {
			// A fabrik table already exists - so we can copy the formatting of its elements
			/** @var ModelList $groupListModel */
			$groupListModel = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('List', 'Site');
	...
	/**
	 * Take an existing table name and create elements for all of its fields
	 * This function belong here
	 *
	 * @param   int    $groupId   group id
	...
	protected function makeElementsFromFields($groupId, $tableName) {
		...
		$input = $this->app->input;
		$elementModel = new PluginelementModel($dispatcher);
		$pluginManager = FabrikWorker::getPluginManager();
...
//print_r("fields = ");print_r($fields);exit;
		foreach ($fields as $label => $properties) {
			...
			if (preg_match("/\((.*)\)/i", $type, $matches)) {
				$maxLength = FabrikArray::getValue($matches, 1, 255);
			...
			// Get the basic type
			$type = explode(" ", $type);
			$type = FabrikArray::getValue($type, 0, '');
			$type = preg_replace("/\((.*)\)/i", '', $type);
			$element = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createTable('Element', 'Administrator');
//			$element = FabTable::getInstance('Element', 'Table');
			...
			$element->label = FabrikArray::getValue($elementLabels, $ordering, str_replace("_", " ", $label));
			...
			case '5':
				...
				break;
			case '6':
				$element->label = FabrikArray::getValue($elementLabels, $ordering, $label);
				break;
			default:
				break;
			}
			...
			// Set the (required) element plugins used here
	 ...
	 * records
	 * This is the 'copy form' feature and should be in admin FormModel
	 *
	...
	private function createLinkedForm($formId = 0) {
		$this->getFormModel(); // Load up a front end form model
		if ($formId == 0) {
			// Fill a new record with data for a newly created form
			/**
	...
	public function createLinkedGroup($data, $isJoin = false, $isRepeat = false) {
		...
		$group->set('published', FabrikArray::getValue($data, 'published', 1));
		$opts = FabrikArray::getValue($data, 'params', new \stdClass );
...
// is set as decimal, why??
	...
	public function copy() {
		...
		foreach ($pks as $i => $pk) {
			...
			// F5: Need to test! Check for "fabrik5" and change params 'list_search_elements' & 'csv_elements'
			if(array_key_exists('fabrik5', (array)$params)) {
				$toMaps = array('list_search_elements', 'csv_elements');
				foreach ($toMaps as $toMap) {
					$key = $toMap;
					$orig = json_decode($params->$key);
					$new = array();
					foreach ($orig->$key as $elementId) {
				...
				foreach ($toMaps as $toMap) {
					$key = $toMap[0];// = list_search_elements
					$key2 = $toMap[1];// = search_elements
					$orig = json_decode($params->$key);
					$new = array();
	...
	private function deleteAssociatedGroups(&$form, $deleteElements = false) {
		...
		// Delete groups
		$groupModel = Factory::getApplication()->bootComponent('com_fabrik')->getMVCFactory()->createModel('Group', '');
	...
	public function dbTableFromXML($key, $name, $xml) {
		...
		foreach ($row->children() as $child) {
			...
			if ($type == '') {
				...
				switch (FabrikString::strtolower($type)) {
				case 'integer':
					$objType = 'INT';
					break;
				case 'datetime':
	 ...
	 */
//	TODO: change $fields = array('id' => 'internalid', 'date_time' => 'jdate') => $fields = array('internalid' => 'id', 'jdate' => 'date_time')
	public function createDBTable($dbTableName = null, $fields = array('id' => 'internalid'), $opts = array()) {
		$db = FabrikWorker::getDbo(true);
		$fabrikDb = $this->getDb();
	...
	public function makeElement($name, $data) {
		$pluginManager = FabrikWorker::getPluginManager();
		$element = $pluginManager->loadPlugIn($data['plugin'], 'element'); // element = site Pluginelementmodel
		$item = $element->getDefaultProperties($data); // Some are ok, but we always need to get (default) plugin params from xml
		// get function getDefaultProperties in site PluginelementModel
		// need to load the plugin xml form and get params from it. also set value as in getDefaultProperties
		// just call the same function as normal add new element
		// use function getForm in admin ElementModel
		// $form = $this->loadForm('com_fabrik.list', 'list', array('control' => 'jform', 'load_data' => $loadData));
	 ...
	 */
// Why do that here? Defaults are in the xml form
	...
	public function ammendTable() {
		...
		$jForm = $input->get('jform', array(), 'array');
		$arGroups = FabrikArray::getValue($jForm, 'current_groups', array(), 'array');
