...
for ($i = 0; $i < count($this->newHeadings); $i++):
	...
	foreach ($this->headings as $sKey => $sVal):
		if (FabrikString::strtolower($heading) == FabrikString::strtolower($sVal)):
...
<?php endif;?>
	...
	<input type="hidden" name="connection_id" value="<?php echo FabrikArray::getValue($jform, 'connection_id') ?>" />
	<input type="hidden" name="jform[addkey]" value="<?php echo FabrikArray::getValue($jform, 'addkey'); ?>" />
	<input type="hidden" name="label" value="<?php echo FabrikArray::getValue($jform, 'label') ?>" />
	<input type="hidden" name="db_table_name" value="<?php echo FabrikArray::getValue($jform, 'db_table_name') ?>" />
