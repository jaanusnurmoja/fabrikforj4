...
namespace Fabrik\Plugin\Element\Date\Extension;
 ...
 * @since       3.0
 */
class Date extends ElementlistModel implements SubscriberInterface
{
    ...
    {
        $pluginMethods = [
        	"onAjax_getAllowedDates" => "onAjax_getAllowedDates",
        	"onCopyRow" => "onCopyRow",
	...
	public function prepareCSVData(&$data, $key, $isRaw = false)
	{
		...
		if ($csvFormat === '0' || $csvFormat === '3')
		{
			...
			for ($j = 0; $j < count($data); $j++)
			{
				...
				try
				{
					$date           = JDate::createFromFormat($format, $orig_data);
	...
	public function formJavascriptClass()
	{
		...
		if ($params->get('date_advanced', '0') == '1' && !in_array('lib/datejs/date', $deps))
		{
			...
			$globalAsset = "lib.datejs.globalization." . Factory::getApplication()->getLanguage()->getTag();
			$wa->registerAndUseScript("com_fabrik." . $globalAsset, "com_fabrik/" . str_replace('.', '/', $globalAsset) . ".js");
		}
 ...
 * @since    3.0
 */
class FaboldDate extends JDate
{
	/**
	 * GMT Date
	 *
