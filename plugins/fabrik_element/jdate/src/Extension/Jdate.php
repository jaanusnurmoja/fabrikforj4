 ...
 */
	protected function timeButton($timeElName, $time, &$str) 
	{
		...
		FabrikHtml::addPath(COM_FABRIK_BASE . 'plugins/fabrik_element/date/images/', 'image', 'form', false);
		...
		$layoutData = (object) array(
			'class' => 'timeButton',
			'label' => FabrikHtml::image($file, 'form', @$this->tmpl, $opts),
			'aria' => ' aria-label="' . Text::_('PLG_ELEMENT_JDATE_ARIA_LABEL_TIME') . '"',
		);
 ...
 */
	...
	public function getEmailValue($value, $data = array(), $repeatCounter = 0) 
	{
		...
		if (is_array($value)) {
			...
			if ($time !== '') {
				$bits = explode(':', $time);
				$h = FabrikArray::getValue($bits, 0, 0);
				$m = FabrikArray::getValue($bits, 1, 0);
				$s = FabrikArray::getValue($bits, 2, 0);
				$d->setTime($h, $m, $s);
			}
	...
	public function calendar($value, $name, $id, $format = 'Y-m-d', $attribs = null, $repeatCounter = 0) 
	{
		...
		return HTMLHelper::calendar($value, $name, $id, $format, $attribs);// F5: also loads calendar.js
	}
...
//		$opts->button = $id . "_cal_img";
	...
	public function elementJavascript($repeatCounter) 
	{
		...
		$opts->timelabel = Text::_('PLG_ELEMENT_JDATE_TIME_LABEL', true);
...
//		$local = $formModel->hasErrors() || $params->get('jdate_store_as_local', 0) == 1) && $value != 'now' ? false : true;
	...
	protected function autoCompleteFilter($default, $v, $labelValue = null, $normal = true, $container = null) 
	{
		...
		FabrikHtml::autoComplete($autoId, $this->getElement()->id, $this->getFormModel()->getId(), 'date');
	...
	protected function singleFilter($default, $v, $type = 'text') 
	{
		...
		$from->img = FabrikHtml::image('calendar', 'form', @$this->tmpl, $imageOpts);
		...
		$displayData->jCal = $this->calendar($rawValue, $v, $this->getFilterHtmlId(0), $format, $this->filterCalendarOpts(), 0);
	...
	protected function rangeFilter($default, $v) 
	{
		...
		$displayData->from = $from;
		$displayData->jCalFrom = $this->calendar($rawValues[0], $v . '[0]', $this->getFilterHtmlId(0), $format, $this->filterCalendarOpts(), 0);
		...
		$imageOpts = array('alt' => 'calendar');
		$to->img = FabrikHtml::image('calendar', 'form', @$this->tmpl, $imageOpts);

		$displayData->to = $to;
		$displayData->filterType = $this->getFilterType();
		$displayData->jCalTo = $this->calendar($rawValues[1], $v . '[1]', $this->getFilterHtmlId(1), $format, $this->filterCalendarOpts(), 0);
	 ...
	 * @return  void
	 */
// F5: called in site ListModel 5902 for normal page load.
	public function filterJS($normal, $container) 
	{
		$element = $this->getElement();
		// F5: $this->getFilterType() gets a strange value somewhere. Need to find where.
		if($this->getFilterType() == 'range') {
			$type = 'range';
		} else { 
			$type = 'field';
		}
...
//		$opts->buttons = $type == 'field' ? array($id . '_cal_img') : array($id . '_cal_img', $id2 . '_cal_img');
		...
		// F5: Only called by advanced search filter, which is an ajax call
