...
class Fileupload extends PluginelementModel implements SubscriberInterface
{
    ...
    {
        $pluginMethods = [
        	"onAjax_clearFileReference" => "onAjax_clearFileReference",
        	"onAjax_upload" => "onAjax_upload",
        	"onDeleteRows" => "onDeleteRows",
        	"onAjax_download" => "onAjax_download",
	...
	public function formJavascriptClass()
	{
		...
		if ($wa->assetExists('script', "plg.fabrik_element.fileupload") === false) {
			$wa->getRegistry()->addRegistryFile("/media/fabrik/plg_fabrik_element_fileupload/joomla.asset.json");
		}
		...
		if ($this->isAjax())
		{
			...
			{
				$wa->useScript("plg.fabrik_element.fileupload.lib.plupload.html5");
			}

			if (strstr($runtimes, 'html4'))
			{
				$wa->useScript("plg.fabrik_element.fileupload.lib.plupload.html4");
			}

			if (strstr($runtimes, 'flash'))
			{
				$wa->useScript("plg.fabrik_element.fileupload.lib.plupload.flash");
			}

			if (strstr($runtimes, 'silverlight'))
			{
				$wa->useScript("plg.fabrik_element.fileupload.lib.plupload.silverlight");
			}

			if (strstr($runtimes, 'browserplus'))
			{
				$wa->useScript("plg.fabrik_element.fileupload.lib.plupload.browserplus");
			}
		}
	...
	protected function _renderListData($data, &$thisRow, $i = 0)
	{
		...
		if ($use_download_script == FU_DOWNLOAD_SCRIPT_TABLE || $use_download_script == FU_DOWNLOAD_SCRIPT_BOTH)
		{
			...
			else
			{
				...
				$displayData->noAccessImage = ($noAccessImage && File::exists(JPATH_SITE .'/media/fabrik/com_fabrik/images/' . $noAccessImage)) ? COM_FABRIK_LIVESITE . 'media/fabrik/com_fabrik/images/' . $noAccessImage : '';
				$displayData->noAccessURL   = $params->get('fu_download_noaccess_url', '');
				$displayData->downloadImg   = ($downloadImg && File::exists(JPATH_SITE .'/media/fabrik/com_fabrik/images/' . $downloadImg)) ? COM_FABRIK_LIVESITE . 'media/fabrik/com_fabrik/images/' . $downloadImg : '';
			...
			if (!empty($ajaxFiles))
			{
				...
				foreach ($fileData as $i => $f)
				{
					...
					if ($file['name'] != '')
					{
						...
						if ($groupModel->canRepeat())
						{
							foreach ($formModel->formData[$name] as $repeatCounter => $files)
							{
								foreach (['id', 'cropdata', 'crop'] as $key)
								{
