...
namespace Fabrik\Plugin\Fabrik_element\Field\Extension;
 ...
 * @since       3.0
 */
class Field extends \PlgFabrik_Element implements SubscriberInterface
{
    ...
    { 
        ...
        $this->setStructure(PluginStructure::J4);
    ...
    protected function _guessLinkType(&$value, $data)
    {
        ...
        if ($params->get('guess_linktype') == '1') {
            ...
            if ((new MediaHelper())->isImage($value)) {
                ...
                $label = FabrikArray::getValue($opts, 'title', '') !== '' ? $opts['title'] : $value;

                $value = FabrikHtml::a($value, $label, $opts);
            }
        }
    }
    ...
    public function elementJavascript($repeatCounter)
    {
        ...
        if ($this->getParams()->get('autocomplete', '0') == '2') {
            ...
            FabrikHtml::autoComplete($id, $this->getElement()->id, $this->getFormModel()->getId(), 'field', $autoOpts);
        }
    ...
    protected function qrCodeLink($thisRow)
    {
        ...
        if (empty($rowId)) {
            ...
            $rowId = FabrikArray::getValue($thisRow, '__pk_val', '');
