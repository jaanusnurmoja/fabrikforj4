...
class Googlemap extends PluginelementModel implements SubscriberInterface
{
	...
	public function onRenderAdminSettings($data = array(), $repeatCounter = null, $mode = null, $subformprefix = null)
	{
		...
		if ($ajax) {
			...
			$wr = $wa->getRegistry();
			$wr->addRegistryFile("/media/fabrik/plg_fabrik_element_googlemap/joomla.asset.json");
			$wa->usePreset("plg.fabrik_element.googlemap");
			// we need to find the modals and then insert the cod to initialize them
			$dom = new \DOMDocument();
			$dom->loadHTML($html);
			$xpath = new \DOMXPath($dom);
			$modalNodes = $xpath->query("//div[@role='dialog']/@id");
			$modalIds = [];
            foreach($modalNodes as $node) {
            	$modalIds[] = $node->nodeValue;
            }
            if (count($modalIds) > 0) {
            	$js = [];
	...
	public function elementJavascript($repeatCounter)
	{
		...
		$opts->radius_resize_icon = COM_FABRIK_LIVESITE . 'media/fabrik/com_fabrik/images/radius_resize.png';
		$opts->radius_resize_off_icon = COM_FABRIK_LIVESITE . 'media/fabrik/com_fabrik/images/radius_resize.png';
