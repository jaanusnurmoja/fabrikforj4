...
window.FbGoogleMap = new Class({
    ...
    updateLatLngElements: function () {
        ...
        if (latEl && lonEl) {
            if (this.options.lat_element.test(/_ddm$/) && this.options.lon_element.test(/_ddm$/)) {
                var latDDM = this.latDecToDDM();
                latEl.update(latDDM);
                var lonDDM = this.lonDecToDDM();
                lonEl.update(lonDDM);

                var latElDec = this.form.formElements.get(this.options.lat_element.replace(/_ddm$/, ''));
                var lonElDec = this.form.formElements.get(this.options.lon_element.replace(/_ddm$/, ''));

                if (latElDec && lonElDec) {
                ...
                var latElDDM = this.form.formElements.get(this.options.lat_element + '_ddm');
                var lonElDDM = this.form.formElements.get(this.options.lon_element + '_ddm');

                if (latElDDM && lonElDDM) {
                    var latDDM = this.latDecToDDM();
                    latElDDM.update(latDDM);
                    var lonDDM = this.lonDecToDDM();
                    lonElDDM.update(lonDDM);
                }
            }
        }
    },
    ...
    updateFromLatLngDDMElements: function () {
        var latElDDM = this.form.formElements.get(this.options.lat_element);
        var lonElDDM = this.form.formElements.get(this.options.lon_element);

        if (latElDDM && lonElDDM) {
            var lat = this.strDDMtoDec(latElDDM.getValue());
            var lon = this.strDDMtoDec(lonElDDM.getValue());

            if (lat !== '' && lon !== '') {
                var pnt = new google.maps.LatLng(lat.toFloat(), lon.toFloat());
                this.marker.setPosition(pnt);
                this.doSetCenter(pnt, this.map.getZoom(), true);

                if (this.options.lat_element.test(/_ddm$/) && this.options.lon_element.test(/_ddm$/)) {
                    var latEl = this.form.formElements.get(this.options.lat_element.replace(/_ddm$/, ''));
                    var lonEl = this.form.formElements.get(this.options.lon_element.replace(/_ddm$/, ''));
    ...
    latDecToDDM: function () {
        var latdec = this.marker.getPosition().lat();
        var dmmlat_d = parseInt(latdec);
        var dmmlat_m_float = (60 * (Math.abs(latdec) - Math.abs(dmmlat_d).toFloat())).toFixed(6).toFloat();

        return dmmlat_d.toString() + ' ' + dmmlat_m_float.toString();
    },

    lonDecToDDM: function () {
        var londec = this.marker.getPosition().lng();
        var dmmlon_d = parseInt(londec);
        var dmmlon_m_float = (60 * (Math.abs(londec) - Math.abs(dmmlon_d).toFloat())).toFixed(6).toFloat();

        return dmmlon_d.toString() + ' ' + dmmlon_m_float.toString();
    },
