...
namespace Fabrik\Plugin\List\Candeleterow\Extension;
 ...
 * @since       3.0
 */
class Candeleterow extends PluginlistModel implements SubscriberInterface
{
    ...
    {
        $pluginMethods = ["onCanDelete" => "onCanDelete"];
	...
	public function onCanDelete($row)
	{
		...
		if (!empty($canDeleteRowEval))
		{
			...
			$canDeleteRowEval = FabrikPhp::Eval(['code' => $canDeleteRowEval, 'vars'=>['data'=>$data]]);
