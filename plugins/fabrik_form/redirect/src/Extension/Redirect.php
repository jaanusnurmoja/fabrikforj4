...
namespace Fabrik\Plugin\Form\Redirect\Extension;
 ...
 * @since       3.0
 */
class Redirect extends PluginformModel implements SubscriberInterface
{
    ...
    {
        $pluginMethods = [
        	"onLastProcess" => "onLastProcess",
        	"onDisplayThanks" => "onDisplayThanks"
        ];
...
isClient('administrator'))
		...
		if (FabrikArray::getValue($this->data, 'thanks_message', '') !== '')
		{
...
<p><?php echo $message ?></p>
	...
	protected function shouldRedirect($params)
	{
		 ...
		 */
		if (FabrikArray::getValue($this->formModel->formData, 'noredirect', 0) !== 0)
		{
			return false;
		}
	...
	protected function getNavIds()
	{
		...
		$o = new \stdClass;
		$o->index = FabrikArray::getValue($keys, $formModel->getRowId(), 0);
		$o->first = $rows[0];
