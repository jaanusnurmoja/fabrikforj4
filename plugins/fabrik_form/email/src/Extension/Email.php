...
namespace Fabrik\Plugin\Form\Email\Extension;
 ...
 * @since       3.0
 */
class Email extends PluginformModel implements SubscriberInterface
{
	...
	public function onAfterProcess()
	{
		...
		if (!empty($emailToEval))
		{
			...
			Worker::clearEval();
			$emailToEval = FabrikPhp::Eval(['code' => $emailToEval, 
		...
		if (!empty($customHeadersEval))
		{
			...
			$customHeaders = FabrikPhp::Eval(['code' => $customHeadersEval]);
	...
	protected function pdfAttachment(&$thisAttachments)
	{
		...
		try
		{
			...
			if ($pdfbootstrap) FabrikHtml::loadBootstrapCSS(true);
	...
	protected function addAttachments()
	{
		...
		{
			Worker::clearEval();
			$email_attach_array = FabrikPhp::Eval(['code' => $emailAttachEval, 'vars' => ['formModel'=>$formModel], 
										'thisVars' => ['data' => $this->data]]);
