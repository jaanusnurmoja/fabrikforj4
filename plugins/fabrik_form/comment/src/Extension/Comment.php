...
namespace Fabrik\Plugin\Form\Comment\Extension;
 ...
 * @since       3.0
 */
class Comment extends PluginformModel implements SubscriberInterface
{
	/**
	 * HTML comment form
	 *
	...
	protected function loadThumbJsOpts()
	{
		...
		FabrikHtml::addPath(COM_FABRIK_BASE . 'plugins/fabrik_element/thumbs/images/', 'image', 'form', false);
	...
	protected function _internal()
	{
		...
		$wa->usePreset("plg.fabrik_form.comment", ["position" => "after"], [], ['com_fabrik.site.fabrik']);
		...
		$wa->addInlineScript("var comments = new FabrikComment('fabrik-comments', $opts);");

		if ($this->doThumbs())
		{
			$wa->addInlineScript("\n comments.thumbs = new FbThumbsList(" . $this->formModel->getId() . ", $thumbOpts);",
				["position" => "after"], [], ["plg.fabrik_element.thumbs"]);
		}
	...
	{
		FabrikHtml::addPath(COM_FABRIK_BASE . 'plugins/fabrik_form/comment/images/', 'image', 'form', false);
		$input = $this->app->input;
		$layoutData = new \stdClass;
		$layoutData->insrc = FabrikHtml::image("star_in.png", 'form', @$this->tmpl, array(), true);
