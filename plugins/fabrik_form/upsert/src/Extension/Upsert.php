...
namespace Fabrik\Plugin\Form\Upsert\Extension;
 ...
 * @since       3.0.7
 */
class Upsert extends PluginformModel implements SubscriberInterface
{
	...
	public function onAfterProcess()
	{
		...
		$origData = FabrikArray::getValue($origData, 0, new \stdClass);
	...
	protected function upsertData($upsertRowExists = false)
	{
		...
		for ($i = 0; $i < count($upsert->upsert_key); $i++)
		{
			...
			if ($upsert->upsert_eval_value[$i] === '1')
			{
				...
				$res = FabrikPhp::Eval(['code' => $v, 'vars'=>['formModel'=>$formModel]]);
