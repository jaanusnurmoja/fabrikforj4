...
namespace Fabrik\Plugin\Form\Juser\Extension;
...
class JUser extends \plgFabrik_Form implements SubscriberInterface
{
    ...
    public static function getSubscribedEvents(): array
    {
        $pluginMethods = [
        	"onLoad" => "onLoad",
        	"onBeforeTruncate" => "onBeforeTruncate",
        	"onDeleteRowsForm" => "onDeleteRowsForm",
	...
	public function onLoad()
	{
		...
		if ($rowId > 0 || $loadCurrentUser)
		{
			...
			$userId = $loadCurrentUser ? Factory::getUser()->id : null;
			...
			if ($params->get('juser_sync_on_edit', 0) == 1)
			{
				...
				$userId            = FabrikArray::getValue($formModel->data, $this->useridfield . '_raw');
				/**
				...
				if (is_array($userId))
				{
					$userId = FabrikArray::getValue($userId, 0, 0);
				}
				...
				if ($userId > 0)
				{
					// See https://github.com/Fabrik/fabrik/issues/1026 - don't use Factory as this loads in session stored user
	...
	public function onBeforeStore()
	{
		...
		if ($params->get('juser_field_userid') != '')
		{
			...
			else
			{
				...
				{
					$originalId = FabrikArray::getValue($originalId, 0, 0);
				}
			}
			// }
		}
		...
		{
			$this->blockfield = $this->getFieldName('juser_field_block');
			$blocked          = FabrikArray::getValue($formModel->formData, $this->blockfield, '');
		...
		if (!$user->bind($data))
		{
			$this->app->enqueueMessage(Text::_('CANNOT BIND THE USER INFORMATION'), 'message');
	...
	protected function trimNamePassword($user, &$data)
	{
		...
		{
			$data['password'] = StringHelper::substr($data['password'], 0, 100);
		}
