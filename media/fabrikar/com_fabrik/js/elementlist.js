...
export class FbElementList extends FbElement{
	...
	addSubClickEvents () {
		...
		this._getSubElements().forEach(function (el) {
			el.addEventListener('click', function (e) {
				Fabrik.fireEvent('fabrik.element.click', [self, e]);
			});
		});
	}
	...
	addNewEvent (action, js) {
		...
		if (action === 'load') {
			...
			if (this.form.events[action] === 'null') {
			...
			if (this.form.events[action][uid] === 'null') {
				...
				jQuery(c).on(action, delegate, function (event) {
					...
					if (target.getAttribute('tagName') === 'LABEL') {
					...
					elid = target.closest('.fabrikSubElementContainer').getAttribute('id');
					that = this.form.formElements[elid];
					subEls = that._getSubElements();
					if (target.length > 0 && subEls.includes(target[0])) {
	...
	startAddNewOption () {
		...
		var l = c.querySelector('input[name=addPicklistLabel]'); // for elements dropdown & picklist
		var v = c.querySelector('input[name=addPicklistValue]');
		var label = l.value;
		if (v) {
			val = v.value;
		} else {
		...
		if (val === '' || label === '') {
			window.alert(Joomla.Text._('PLG_ELEMENT_CHECKBOX_ENTER_VALUE_LABEL')); // only for element checkbox
		}
		else {
			...
			r.querySelector('.' + this.type + ' span').set('text', label);
			...
			document.getElementById(this.form.form).fireEvent('change', {target: is[index]});
	...
	watchAdd () {
		...
		if (this.options.allowadd === true && this.options.editable !== false) {
			...
			c.getElements('input[name=addPicklistLabel], input[name=addPicklistValue]').addEventListener('keypress', function (e) {
				self.checkEnter(e);
			});
			c.querySelector('input[type=button]').addEventListener('click', function (e) {
				e.stop();
