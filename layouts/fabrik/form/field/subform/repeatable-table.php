...
if (!empty($groupByFieldset)) {
    ...
    foreach ($tmpl->getGroup('') as $field) {
        $table_head .= '<th scope="col" class="col-auto">' . strip_tags($field->label);
	...
	// Fabrik: Needed to support col-auto
    Factory::getApplication()
        ->getDocument()
        ->addStyleDeclaration('tbody.subform-repeatable-container tr.subform-repeatable-group td .control-group .controls {min-width: 0;}');
}
?>
...
<div class="subform-repeatable-wrapper subform-table-layout subform-table-sublayout-<?php echo $sublayout; ?>">
    <joomla-field-subform class="subform-repeatable<?php echo $class; ?>" name="<?php echo $name; ?>"
        ...
        <div class="table-responsive"> <?php // Fabrik: Does not work in list setting. Does work in menu and module.  ?>
            <table class="table table-sm" id="subfieldList_<?php echo $fieldId; ?>">
        ...
        <?php if ($multiple) : ?>
        <template class="subform-repeatable-template-section hidden"> <?php // gives a console error: The row template is required for the subform element to work ?>
